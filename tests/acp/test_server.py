"""Tests for acp_adapter.server — SparkACPAgent ACP server."""
...
from acp_adapter.server import SparkACPAgent, SPARK_VERSION
...
def agent(mock_manager):
    """SparkACPAgent backed by a mock session manager."""
    return SparkACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "spark-agent"
        assert resp.agent_info.version == SPARK_VERSION
...
class TestSlashCommands:
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("spark_cli.config.load_config", lambda: {
        ...
        with patch("core.run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = SparkACPAgent(session_manager=manager)
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["spark-acp"]
        ...
        with patch("tools.mcp_tool.register_mcp_servers", side_effect=capture_register), \
             patch("core.model_tools.get_tool_definitions", return_value=[]):
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        with patch("tools.mcp_tool.register_mcp_servers", return_value=["mcp_srv_search"]), \
             patch("core.model_tools.get_tool_definitions", return_value=fake_tools):
            await agent._register_session_mcp_servers(state, [server])

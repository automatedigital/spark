"""
Tests for timezone support (spark_time module + integration points).
...
import core.spark_time as spark_time


def _reset_spark_time_cache():
    """Reset the spark_time module cache (replacement for removed reset_cache)."""
    spark_time._cached_tz = None
    spark_time._cached_tz_name = None
    spark_time._cache_resolved = False


# =========================================================================
# spark_time.now() — core helper
# =========================================================================

class TestSparkTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_spark_time_cache()

    def teardown_method(self):
        _reset_spark_time_cache()
        os.environ.pop("SPARK_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["SPARK_TIMEZONE"] = "Asia/Kolkata"
        result = spark_time.now()
    ...
    def test_us_eastern(self):
        ...
        os.environ["SPARK_TIMEZONE"] = "America/New_York"
        result = spark_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["SPARK_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="spark_time"):
            result = spark_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["SPARK_TIMEZONE"] = "Europe/London"
        tz = spark_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["SPARK_TIMEZONE"] = "Not/A/Timezone"
        tz = spark_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When SPARK_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["SPARK_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When SPARK_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("SPARK_TIMEZONE", None)
    ...
    def test_spark_timezone_not_leaked_to_child(self):
        """SPARK_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["SPARK_TIMEZONE"] = "Asia/Kolkata"

        with patch("core.model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("SPARK_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=spark_tz) which shifted
        absolute time when system-local tz != Spark tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_spark_tz(self):
        """Already-aware datetimes should be normalized to Spark tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Spark tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Spark tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Spark is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Spark tz
        ...
        # Use a Spark timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _spark_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["SPARK_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_spark_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Spark timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["SPARK_TIMEZONE"] = "US/Eastern"
        _reset_spark_time_cache()

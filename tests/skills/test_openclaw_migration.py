...
pytestmark = pytest.mark.skip(reason="optional-skills bundle not included in this repo")
...
def load_skills_guard():
    spec = importlib.util.spec_from_file_location(
        "skills_guard_local",
        Path(__file__).resolve().parents[2] / "src" / "tools" / "skills_guard.py",
    )
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules[spec.name] = module
...
"""
    ...
    assert "Tyler Williams > Active Projects: Spark Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `spark cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # spark_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.spark/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "spark_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Spark prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Spark to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Spark config is great"
    assert mod.rebrand_text("openclaw should always respond concisely") == "Spark should always respond concisely"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Spark uses tools well"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Spark remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "Spark prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Spark was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "Spark uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Spark said to ask Spark about Spark settings"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Spark" in result

...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure SPARK_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", spark_home=str(tmp_path / ".spark"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", spark_home=profile_home)
        assert p._client.project == "spark-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".spark" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        spark_home = tmp_path / ".spark"
        spark_home.mkdir(exist_ok=True)
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", spark_home=str(spark_home))
        return p

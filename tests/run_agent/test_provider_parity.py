...
def _make_agent(monkeypatch, provider, api_mode="chat_completions", base_url="https://openrouter.ai/api/v1", model=None):
    monkeypatch.setattr("core.run_agent.get_tool_definitions", lambda **kw: _tool_defs("web_search", "terminal"))
    monkeypatch.setattr("core.run_agent.check_toolset_requirements", lambda: {})
    monkeypatch.setattr("core.run_agent.OpenAI", _FakeOpenAI)
    kwargs = dict(
        api_key="test-key",
        base_url=base_url,
        provider=provider,
...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_nous_portal(self, monkeypatch):
        agent = _make_agent(monkeypatch, "nous", base_url="")
        agent.model = "gpt-5"
        messages = [
            {"role": "system", "content": "You are helpful."},
            {"role": "user", "content": "hi"},
...
class TestBuildApiKwargsNousPortal:
    def test_includes_nous_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == ["product=spark-agent"]
...
# ── Chat completions response handling (OpenRouter/Spark Portal) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_custom_endpoint_when_no_nous(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Spark Portal keys are available.

...
class TestTryActivateFallback:
    ...
    def test_activates_nous_fallback(self):
        """Spark Portal fallback should use OAuth credentials and chat_completions mode."""
        agent = _make_agent(
            fallback_model={"provider": "nous", "model": "nous-spark-3"},
        )
        mock_client = _mock_resolve(
        ...
        with patch(
            "agent.auxiliary_client.resolve_provider_client",
            return_value=(mock_client, "nous-spark-3"),
        ):
            result = agent._try_activate_fallback()
            assert result is True
            assert agent.model == "nous-spark-3"
            assert agent.provider == "nous"
            assert agent.api_mode == "chat_completions"
            assert agent.client is mock_client

    def test_nous_fallback_fails_gracefully_without_login(self):
        """Spark Portal fallback should return False if not logged in."""
        agent = _make_agent(
            fallback_model={"provider": "nous", "model": "nous-spark-3"},
        )
        with patch(

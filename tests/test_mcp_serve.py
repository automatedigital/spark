"""
Tests for mcp_serve — Spark MCP server.
...
# mcp_serve.py lives in scripts/, which is not a package — add it to path
...
def _isolate_spark_home(tmp_path, monkeypatch):
    """Redirect SPARK_HOME to a temp directory."""
    monkeypatch.setenv("SPARK_HOME", str(tmp_path))
    try:
        import core.spark_constants as spark_constants
        monkeypatch.setattr(spark_constants, "get_spark_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking spark_state schema."""

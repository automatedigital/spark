...
class TestGatewayIntegration(unittest.TestCase):
    def test_feishu_in_adapter_factory(self):
        source = Path("src/gateway/run.py").read_text(encoding="utf-8")
    ...
    def test_feishu_toolset_exists(self):
        ...
        self.assertIn("spark-feishu", TOOLSETS)
        self.assertIn("spark-feishu", TOOLSETS["spark-gateway"]["includes"])
        ...
        class _ChatAPI:
            def get(self, request):
                ...
                return SimpleNamespace(
                    success=lambda: True,
                    data=SimpleNamespace(name="Spark Group", chat_type="group"),
                )
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_group_post_message_uses_parsed_mentions_when_sdk_mentions_missing(self):
        ...
        message = SimpleNamespace(
            message_type="post",
            mentions=[],
            content='{"en_us":{"content":[[{"tag":"at","user_name":"Spark","open_id":"ou_bot"}]]}}',
        )
    ...
    @patch.dict(
        os.environ,
        {
            "SPARK_FEISHU_TEXT_BATCH_MAX_MESSAGES": "2",
        },
        clear=True,
    )
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"SPARK_HOME": temp_home}, clear=False):

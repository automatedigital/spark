...
async def test_none_user_id_does_not_generate_pairing_code(monkeypatch, tmp_path):
    ...
    adapter = SimpleNamespace(send=AsyncMock())
    runner.adapters[Platform.DISCORD] = adapter
    runner.pairing_store._is_rate_limited = lambda *_args, **_kwargs: False
    runner.pairing_store.generate_code = lambda *_args, **_kwargs: "ABCD1234"
    runner.pairing_store._is_rate_limited = lambda *_args, **_kwargs: False
    runner.pairing_store.generate_code = lambda *_args, **_kwargs: "ABCD1234"
...
async def test_non_internal_event_without_user_triggers_pairing(monkeypatch, tmp_path):
    ...
    # module-level dotenv in gateway/run.py from the real ~/.spark/.env).

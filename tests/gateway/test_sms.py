...
class TestSmsToolset:
    def test_spark_sms_toolset_exists(self):
        from core.toolsets import get_toolset

        ts = get_toolset("spark-sms")
        assert ts is not None
        assert "tools" in ts

    def test_spark_sms_in_gateway_includes(self):
        from core.toolsets import get_toolset

        gw = get_toolset("spark-gateway")
        assert gw is not None
        assert "spark-sms" in gw["includes"]

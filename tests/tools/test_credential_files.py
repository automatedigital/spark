...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        spark_home.mkdir()
        (spark_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(spark_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.spark/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        (spark_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        (spark_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        (spark_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.spark/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        spark_home = tmp_path / ".spark"
        (spark_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SPARK_HOME": str(spark_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.spark")

        assert mounts[0]["container_path"] == "/home/user/.spark/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.spark/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.spark/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SPARK_HOME."""
        monkeypatch.setenv("SPARK_HOME", str(tmp_path / ".spark"))
        (tmp_path / ".spark").mkdir()

        # Create a sensitive file one level above spark_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside spark_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside SPARK_HOME must still be registered."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        (spark_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_spark_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SPARK_HOME must be allowed."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        subdir = spark_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SPARK_HOME pointing outside must be rejected."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))

        # Create a sensitive file outside spark_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside spark_home pointing outside
        symlink = spark_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SPARK_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, spark_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SPARK_HOME."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        ...
        sensitive.write_text("{}")
        self._write_config(spark_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(spark_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SPARK_HOME via config must still mount."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))

        (spark_home / "oauth.json").write_text("{}")
        self._write_config(spark_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        (spark_home / "cache" / "documents").mkdir(parents=True)
        (spark_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("SPARK_HOME", str(spark_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.spark/cache/documents" in paths
        assert "/root/.spark/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.spark/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        # Use legacy dir name — get_spark_dir prefers old if it exists
        (spark_home / "document_cache").mkdir()
        (spark_home / "image_cache").mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(spark_home / "document_cache") in host_paths
        assert str(spark_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.spark/cache/documents" in container_paths
        assert "/root/.spark/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        spark_home = tmp_path / ".spark"
        doc_dir = spark_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        spark_home = tmp_path / ".spark"
        ss_dir = spark_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.spark/cache/screenshots/session_abc/screen1.png"

...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._spark_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_spark_bin(self):
        """After _INSTALL_FAILED, manual install in SPARK_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        spark_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(spark_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(spark_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._spark_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == spark_bin
            assert _tirith_mod._resolved_path == spark_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# SPARK_HOME isolation
# ---------------------------------------------------------------------------

class TestSparkHomeIsolation:
    def test_spark_bin_dir_respects_spark_home(self):
        """_spark_bin_dir must use SPARK_HOME, not hardcoded ~/.spark."""
        from tools.tirith_security import _spark_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SPARK_HOME": tmpdir}):
            result = _spark_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_spark_home(self):
        """_failure_marker_path must use SPARK_HOME, not hardcoded ~/.spark."""
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        assert spark_home is not None, "SPARK_HOME should be set by conftest"
        assert "spark_test" in spark_home, "Should point to test temp dir"

    def test_get_spark_home_fallback(self):
        """Without SPARK_HOME set, falls back to ~/.spark."""
        from tools.tirith_security import _get_spark_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SPARK_HOME entirely
            os.environ.pop("SPARK_HOME", None)
            result = _get_spark_home()
        assert result == os.path.join(os.path.expanduser("~"), ".spark")

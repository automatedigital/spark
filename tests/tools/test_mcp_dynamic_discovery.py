...
class TestRegisterServerTools:
    ...
    def test_injects_spark_toolsets(self, mock_registry, mock_toolsets):
        """Tools are injected into spark-* toolsets but not custom ones."""
        ...
        with patch("tools.registry.registry", mock_registry), \
            ...
            patch.dict("core.toolsets.TOOLSETS", mock_toolsets, clear=True):
        ...
        # Injected into spark-* toolsets
        assert "mcp_my_srv_my_tool" in mock_toolsets["spark-cli"]["tools"]
        assert "mcp_my_srv_my_tool" in mock_toolsets["spark-telegram"]["tools"]
        # NOT into non-spark toolsets
...
class TestRefreshTools:
    ...
    async def test_nuke_and_repave(self, mock_registry, mock_toolsets):
        ...
        mock_toolsets["spark-cli"]["tools"].append("mcp_live_srv_old_tool")
        ...
        assert "mcp_live_srv_old_tool" not in mock_toolsets["spark-cli"]["tools"]

        # New tool registered
        assert "mcp_live_srv_new_tool" in mock_registry.get_all_tool_names()
        assert "mcp_live_srv_new_tool" in mock_toolsets["spark-cli"]["tools"]

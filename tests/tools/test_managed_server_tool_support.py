...
Validates that:
1. The installed atroposlib API still matches Spark's expectations
2. Spark's parser registry remains compatible with ManagedServer parsing
3. SparkAgentBaseEnv wires the selected parser into ServerManager correctly

These tests verify the contract between spark-agent's environments/ code
...
class TestManagedServerAPI:
    """Test that ManagedServer's API matches what spark-agent expects."""
...
class TestParserCompatibility:
    """Test that spark-agent's parsers match ManagedServer's expectations."""
...
class TestBaseEnvCompatibility:
    """Test that spark_base_env.py's tool-parser wiring matches the current API."""

    def test_spark_base_env_sets_server_manager_tool_parser(self):
        """Spark wires parser selection through ServerManager.tool_parser."""
        import ast

        base_env_path = Path(__file__).parent.parent.parent / "environments" / "spark_base_env.py"
        ...
        assert found_assignment, (
            "spark_base_env.py should set self.server.tool_parser from config.tool_call_parser"
        )

    def test_spark_base_env_uses_config_tool_call_parser(self):
        """Verify spark_base_env uses the config field rather than a local parser instance."""
        base_env_path = Path(__file__).parent.parent.parent / "environments" / "spark_base_env.py"
        source = base_env_path.read_text()

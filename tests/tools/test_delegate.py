...
class TestToolNamePreservation(unittest.TestCase):
    ...
    def test_build_child_agent_does_not_raise_name_error(self):
        ...
        with patch("core.run_agent.AIAgent"):
            try:
                _build_child_agent(
                    task_index=0,
...
class TestDelegationCredentialResolution(unittest.TestCase):
    ...
    def test_nous_provider_resolves_nous_credentials(self, mock_resolve):
        """Spark Portal provider resolves Spark Portal base_url and api_key."""
        mock_resolve.return_value = {
            "provider": "nous",
            "base_url": "",
        ...
        parent = _make_mock_parent(depth=0)
        cfg = {"model": "spark-3-llama-3.1-8b", "provider": "nous"}
        creds = _resolve_delegation_credentials(cfg, parent)
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_cross_provider_delegation(self, mock_creds, mock_cfg):
        """Parent on Spark Portal, subagent on OpenRouter — full credential switch."""
        mock_cfg.return_value = {
            "max_iterations": 45,
            "model": "google/gemini-3-flash-preview",
            "provider": "openrouter",
        ...
        with patch("core.run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            # Child should use OpenRouter, NOT Spark Portal
            self.assertEqual(kwargs["provider"], "openrouter")
            self.assertEqual(kwargs["base_url"], "https://openrouter.ai/api/v1")

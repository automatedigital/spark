...
def _clean_state():
    ...
    saved = {}
    for k in ("SPARK_INTERACTIVE", "SPARK_GATEWAY_SESSION", "SPARK_EXEC_ASK", "SPARK_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SPARK_INTERACTIVE"] = "1"
        session_key = os.getenv("SPARK_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SPARK_INTERACTIVE or SPARK_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

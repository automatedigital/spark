...
def _restore_tool_modules():
    original_spark_home = os.environ.get("SPARK_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_spark_home is None:
            os.environ.pop("SPARK_HOME", None)
        else:
            os.environ["SPARK_HOME"] = original_spark_home
        _reset_modules(("tools", "spark_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    spark_cli = types.ModuleType("spark_cli")
    spark_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["spark_cli"] = spark_cli
    spark_home = tmp_path / "spark-home"
    os.environ["SPARK_HOME"] = str(spark_home)
    sys.modules["spark_cli.config"] = types.SimpleNamespace(
        get_spark_home=lambda: spark_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="spark-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": spark_home / "modal_snapshots.json",

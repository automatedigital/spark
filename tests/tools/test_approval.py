...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("spark_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("spark_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"SPARK_SESSION_KEY": "bob"}, clear=False):
    ...
    def test_gateway_runner_binds_session_key_to_context_before_agent_run(self):
        run_py = Path(__file__).resolve().parents[2] / "src" / "gateway" / "run.py"
...
class TestTeePattern:
    ...
    def test_tee_spark_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.spark/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_spark_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $SPARK_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_spark_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$SPARK_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_spark_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $SPARK_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.spark/spark-agent && source venv/bin/activate && python -m spark_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m spark_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m spark_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "spark_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_not_flagged(self):
        ...
        cmd = "systemctl --user restart spark-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_spark_detected(self):
        """pkill targeting.spark/gateway processes must be caught."""
        cmd = 'pkill -f "core.cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_spark_detected(self):
        cmd = "killall spark"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep spark) bypasses the pkill/killall name-matching
    ...
    def test_pkill_spark_still_detected(self):

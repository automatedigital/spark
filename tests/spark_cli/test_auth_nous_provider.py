"""Regression tests for Spark Portal OAuth refresh + agent-key mint interactions."""
...
from spark_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from spark_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("SPARK_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_spark_ca_bundle_env_falls_back(self, monkeypatch):
        from spark_cli.auth import _resolve_verify

        monkeypatch.setenv("SPARK_CA_BUNDLE", "/nonexistent/spark-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (spark_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_nous_auth_status() should find Spark Portal credentials in the pool
    even when the auth store has no Spark Portal provider entry — this is the
    case when login happened via the dashboard device-code flow which
    saves to the pool only.
    """
    from spark_cli.auth import get_nous_auth_status

    spark_home = tmp_path / "spark"
    spark_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Spark Portal provider entry
    (spark_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("SPARK_HOME", str(spark_home))

    # Seed the credential pool with a Spark Portal entry
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    spark_home = tmp_path / "spark"
    _setup_nous_auth(spark_home, access_token="at-123")
    monkeypatch.setenv("SPARK_HOME", str(spark_home))
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    spark_home = tmp_path / "spark"
    _setup_nous_auth(spark_home, refresh_token="refresh-old")
    monkeypatch.setenv("SPARK_HOME", str(spark_home))
    ...
    monkeypatch.setattr("spark_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("spark_cli.auth._mint_agent_key", _fake_mint_agent_key)

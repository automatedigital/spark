...
Ensures ``spark update`` correctly detects running gateways managed by
...
from spark_cli.main import cmd_update
        ...
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="spark-gateway.service loaded active running Spark Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>SPARK_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: spark gateway run'."""
        ...
        assert "Restart manually: spark gateway run" not in captured
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted spark-gateway" in captured
...
class TestServicePidExclusion:
    ...
    where ``spark update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.spark.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="spark-gateway.service loaded inactive dead Spark Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.spark/spark-agent/venv/bin/python -m spark_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.spark/spark-agent/venv/bin/python -m spark_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda spark_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``spark update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point SPARK_HOME at a temp dir so the marker file lands there
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        import spark_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_spark_home", lambda: spark_home)

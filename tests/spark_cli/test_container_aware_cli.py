...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SPARK_HOME with .container-mode file."""
    spark_home = tmp_path / ".spark"
    spark_home.mkdir()
    monkeypatch.setenv("SPARK_HOME", str(spark_home))
    monkeypatch.delenv("SPARK_DEV", raising=False)

    container_mode = spark_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=spark-agent\n"
        "exec_user=spark\n"
        "spark_bin=/data/current-package/bin/spark\n"
    )
    return spark_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("core.spark_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "spark-agent"
    assert info["exec_user"] == "spark"
    assert info["spark_bin"] == "/data/current-package/bin/spark"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("core.spark_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_spark_dev(container_env, monkeypatch):
    """Returns None when SPARK_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SPARK_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_spark_dev_zero(container_env, monkeypatch):
    """SPARK_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SPARK_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        spark_home.mkdir()
        (spark_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("core.spark_constants.is_container", return_value=False), \
             patch("spark_cli.config.get_spark_home", return_value=spark_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SPARK_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=spark-custom\n"
        "exec_user=myuser\n"
        "spark_bin=/opt.spark/bin/spark\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "spark-custom"
    assert info["exec_user"] == "myuser"
    assert info["spark_bin"] == "/opt.spark/bin/spark"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from spark_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "spark"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "spark-agent" in cmd
    assert "/data/current-package/bin/spark" in cmd
    assert "chat" in cmd

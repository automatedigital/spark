"""Tests for spark_cli configuration management."""
...
class TestGetSparkHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            home = get_spark_home()
            assert home == Path.home() / ".spark"

    def test_env_override(self):
        with patch.dict(os.environ, {"SPARK_HOME": "/custom/path"}):
            home = get_spark_home()
            assert home == Path("/custom/path")


class TestEnsureSparkHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"SPARK_HOME": str(tmp_path)}, clear=False):
...
class TestSaveHttpApiEnvBlock:
    def test_writes_defaults_and_replaces_prior_api_server_lines(self, tmp_path):
        with patch.dict(os.environ, {"SPARK_HOME": str(tmp_path)}, clear=False):
            for k in (
                "API_SERVER_ENABLED",
                "API_SERVER_KEY",
            ...
            env_path.write_text(
                ...
                "# API_SERVER_KEY=oldsecret\n"
                "OTHER=keep\n",
                encoding="utf-8",
            )
            save_http_api_env_block("mysecretkey123456")
            text = env_path.read_text(encoding="utf-8")
            assert "OTHER=keep" in text
            assert "API_SERVER_ENABLED=true" in text
            assert "API_SERVER_KEY=mysecretkey123456" in text
            assert "API_SERVER_HOST=127.0.0.1" in text
            assert "API_SERVER_PORT=8642" in text
            assert "oldsecret" not in text
            assert os.environ["API_SERVER_KEY"] == "mysecretkey123456"
            assert os.environ["API_SERVER_PORT"] == "8642"

    def test_rejects_placeholder_keys(self, tmp_path):
        with patch.dict(os.environ, {"SPARK_HOME": str(tmp_path)}):
            (tmp_path / ".env").write_text("X=1\n", encoding="utf-8")
            with pytest.raises(ValueError, match="placeholder"):
                save_http_api_env_block("changeme")
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SPARK_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SPARK_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"SPARK_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SPARK_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from spark_cli.config import ENV_VARS_BY_VERSION
        all_vars = []

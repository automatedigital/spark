...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("SPARK_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("SPARK_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("SPARK_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: spark gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup spark gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.spark.gateway"])
            ),
        )
...
class TestSystemUnitSparkHome:
    """SPARK_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'SPARK_HOME=/home/alice/.spark' in unit
        assert '/root/.spark' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: SPARK_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("SPARK_HOME", "/root/.spark/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'SPARK_HOME=/home/alice/.spark/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_spark_home(self, monkeypatch):
        # Custom SPARK_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("SPARK_HOME", "/opt/spark-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'SPARK_HOME=/opt/spark-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's SPARK_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        spark_home = str(gateway_cli.get_spark_home().resolve())
        assert f'SPARK_HOME={spark_home}' in unit


class TestSparkHomeForTargetUser:
    """Unit tests for _spark_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("SPARK_HOME", raising=False)

        result = gateway_cli._spark_home_for_target_user("/home/alice")
        assert result == "/home/alice/.spark"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._spark_home_for_target_user("/home/alice")
        assert result == "/home/alice/.spark/profiles/coder"
...
class TestProfileArg:
    ...
    def test_default_spark_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.spark should not produce a --profile flag."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        result = gateway_cli._profile_arg(str(spark_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.spark/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".spark" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile SPARK_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "spark"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.spark/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".spark" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".spark" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARK_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_spark_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".spark" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.spark.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".spark" / "spark-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".spark" / "spark-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/spark", str(tmp_path / "alice"))
        assert result == "/opt/spark"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".spark" / "spark-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".spark" / "spark-agent"
        ...
        monkeypatch.setenv("SPARK_HOME", str(root_home / ".spark"))
        monkeypatch.setattr(gateway_cli, "get_spark_home", lambda: root_home / ".spark")
        ...
        assert "WorkingDirectory=/home/alice/.spark/spark-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "spark-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "spark-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'spark gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'spark gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'spark gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "spark gateway run" in out

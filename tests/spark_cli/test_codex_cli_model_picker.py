...
and haven't been migrated to the Spark auth store yet.

Root cause: list_authenticated_providers() checked the raw Spark auth
...
Spark auth store has no openai-codex tokens, and
...
def test_codex_cli_tokens_detected_by_model_picker(codex_cli_only_env):
    ...
    from spark_cli.model_switch import list_authenticated_providers
...
def test_codex_cli_tokens_migrated_after_detection(codex_cli_only_env):
    ...
    migrated into the Spark auth store for subsequent fast lookups."""
    from spark_cli.model_switch import list_authenticated_providers

    # First call triggers migration
    list_authenticated_providers(current_provider="openai-codex")

    # Verify tokens are now in Spark auth store
    auth_path = codex_cli_only_env / "auth.json"
    store = json.loads(auth_path.read_text())
    providers = store.get("providers", {})
    assert "openai-codex" in providers, (
        f"openai-codex not migrated to Spark auth store: {list(providers.keys())}"
    )
...
@pytest.fixture()
def spark_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Spark auth store (no Codex CLI needed)."""
    spark_home = tmp_path / ".spark"
    spark_home.mkdir()
...
def test_normal_path_still_works(spark_auth_only_env):
    """openai-codex appears when tokens are already in Spark auth store."""
    from spark_cli.model_switch import list_authenticated_providers
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Spark
    auth store."""
    spark_home = tmp_path / ".spark"
    spark_home.mkdir()

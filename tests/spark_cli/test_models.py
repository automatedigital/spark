...
class TestFilterNousFreeModels:
    """Tests for filter_nous_free_models — Spark Portal free-model policy."""
...
class TestCheckNousFreeTierCache:
    ...
    @patch("spark_cli.models.fetch_nous_account_tier")
    @patch("spark_cli.models.is_nous_free_tier", return_value=True)
    def test_result_is_cached(self, mock_is_free, mock_fetch):
        ...
        with patch("spark_cli.auth.get_provider_auth_state", return_value={"access_token": "tok"}), \
             patch("spark_cli.auth.resolve_nous_runtime_credentials"):
    ...
    @patch("spark_cli.models.fetch_nous_account_tier")
    @patch("spark_cli.models.is_nous_free_tier", return_value=False)

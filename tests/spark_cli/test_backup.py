"""Tests for spark backup and import commands."""
...
def _make_spark_tree(root: Path) -> None:
    """Create a realistic ~/.spark directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "core.spark_state.db").write_bytes(b"fake-state")
    ...
    # spark-agent repo (should be EXCLUDED)
    (root / "spark-agent").mkdir(exist_ok=True)
    (root / "spark-agent" / "core.run_agent.py").write_text("# big file\n")
    (root / "spark-agent" / ".git").mkdir()
    (root / "spark-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_spark_agent(self):
        from spark_cli.backup import _should_exclude
        assert _should_exclude(Path("spark-agent/run_agent.py"))
        assert _should_exclude(Path("spark-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        spark_home.mkdir()
        _make_spark_tree(spark_home)

        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        # get_default_spark_root needs this
    ...
    def test_excludes_spark_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include spark-agent/ directory."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        _make_spark_tree(spark_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "spark-agent" in n]
            assert agent_files == [], f"spark-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/spark-backup-*.zip."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        (spark_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("spark-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Spark backup."""
        from spark_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only spark_state.db (old wrong name) is rejected."""
        from spark_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["core.spark_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into spark home."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (spark_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (spark_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (spark_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (spark_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_spark_prefix(self, tmp_path, monkeypatch):
        """Import strips .spark/ prefix if all entries share it."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".spark/config.yaml": "model: test\n",
            ".spark/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (spark_home / "config.yaml").read_text() == "model: test\n"
        assert (spark_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_spark_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a spark backup."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (spark_home / "config.yaml").exists()
        # traversal file should NOT exist outside spark home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (spark_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (spark_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (spark_home / "config.yaml").read_text() == "model: restored\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".spark"
        src_home.mkdir(parents=True)
        _make_spark_tree(src_home)

        monkeypatch.setenv("SPARK_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from spark_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".spark"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("SPARK_HOME", str(dst_home))
        ...
        # spark-agent should NOT be present
        assert not (dst_home / "spark-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from spark_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without spark markers fails validation."""
        import io
        from spark_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_spark(self):
        """Detects .spark/ prefix wrapping all entries."""
        import io
        from spark_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".spark/config.yaml", "test")
            zf.writestr(".spark/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".spark/"
...
class TestBackupEdgeCases:
    def test_nonexistent_spark_home(self, tmp_path, monkeypatch):
        """Backup exits when spark home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".spark"
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("spark-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_spark_home(self, tmp_path, monkeypatch):
        """Backup handles empty spark home (no files to back up)."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        # Only excluded dirs, no actual files
        (spark_home / "__pycache__").mkdir()
        (spark_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_skips_output_zip_inside_spark(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside spark root."""
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        (spark_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside spark home
        out_zip = spark_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        (spark_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        spark_home = tmp_path / ".spark"
        spark_home.mkdir()
        (spark_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("SPARK_HOME", str(spark_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (spark_home / "config.yaml").exists()
        assert (spark_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (spark_home / "profiles" / "coder" / "config.yaml").exists()
        assert (spark_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "spark -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import spark_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "spark_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from spark_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def spark_home(self, tmp_path):
        """Create a fake SPARK_HOME with critical state files."""
        home = tmp_path / ".spark"
        home.mkdir()
    ...
    def test_creates_snapshot(self, spark_home):
        from spark_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(spark_home=spark_home)
        assert snap_id is not None
        snap_dir = spark_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, spark_home):
        from spark_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", spark_home=spark_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, spark_home):
        from spark_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(spark_home=spark_home)
        db_copy = spark_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, spark_home):
        from spark_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(spark_home=spark_home)
        assert (spark_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, spark_home):
        from spark_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(spark_home=spark_home)
        with open(spark_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(spark_home=empty) is None

    def test_list_snapshots(self, spark_home):
        from spark_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", spark_home=spark_home)
        id2 = create_quick_snapshot(label="second", spark_home=spark_home)

        snaps = list_quick_snapshots(spark_home=spark_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, spark_home):
        from spark_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", spark_home=spark_home)
        snaps = list_quick_snapshots(limit=3, spark_home=spark_home)
        assert len(snaps) == 3

    def test_restore_config(self, spark_home):
        from spark_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(spark_home=spark_home)

        (spark_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (spark_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, spark_home=spark_home)
        assert result is True
        assert "openrouter" in (spark_home / "config.yaml").read_text()

    def test_restore_state_db(self, spark_home):
        from spark_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(spark_home=spark_home)

        conn = sqlite3.connect(str(spark_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, spark_home=spark_home)
    ...
    def test_restore_nonexistent(self, spark_home):
        from spark_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", spark_home=spark_home) is False

    def test_auto_prune(self, spark_home):
        from spark_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", spark_home=spark_home)
        snaps = list_quick_snapshots(limit=100, spark_home=spark_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, spark_home):
        from spark_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", spark_home=spark_home)
        deleted = prune_quick_snapshots(keep=3, spark_home=spark_home)
        assert deleted == 7
        assert len(list_quick_snapshots(spark_home=spark_home)) == 3

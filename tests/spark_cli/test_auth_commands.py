...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (spark_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "spark" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_spark_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:spark_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:spark_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_nous_is_rejected(tmp_path, monkeypatch):
    monkeypatch.setenv("SPARK_HOME", str(tmp_path / "spark"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    from spark_cli.auth_commands import auth_add_command
    import pytest
    ...
    with pytest.raises(SystemExit, match="Provider 'nous' is no longer supported"):
        auth_add_command(_Args())
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "spark_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from spark_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from spark_cli.auth_commands import auth_reset_command
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from spark_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "spark_cli.auth_commands.load_pool",
        lambda provider: (
            _Pool()
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("spark_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("spark_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("spark_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "spark_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (spark_home / "auth.json").write_text(json.dumps(auth_store))
    ...
    updated = json.loads((spark_home / "auth.json").read_text())

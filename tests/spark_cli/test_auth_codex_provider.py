"""Tests for Codex auth — tokens stored in Spark auth store (~/.spark/auth.json)."""
...
def _setup_spark_auth(spark_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Spark auth store."""
    ...
    auth_file = spark_home / "auth.json"
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    spark_home = tmp_path / "spark"
    _setup_spark_auth(spark_home, access_token="")
    monkeypatch.setenv("SPARK_HOME", str(spark_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_spark_auth(spark_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("SPARK_HOME", str(spark_home))
    ...
    monkeypatch.setattr("spark_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    spark_home = tmp_path / "spark"
    _setup_spark_auth(spark_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("SPARK_HOME", str(spark_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Spark auth store, not ~/.codex/."""
    spark_home = tmp_path / "spark"
    codex_home = tmp_path / "codex-cli"
    spark_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "spark-at", "refresh_token": "spark-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Spark store
    assert not (codex_home / "auth.json").exists()

    # Spark auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "spark-at"
...
def test_resolve_returns_spark_auth_store_source(tmp_path, monkeypatch):
    ...
    assert creds["source"] == "spark-auth-store"

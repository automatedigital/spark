"""Comprehensive tests for spark_cli.profiles module.
...
from spark_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_spark_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and SPARK_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.spark/profiles)
    * SPARK_HOME  -> tmp_path/.spark  (so get_spark_home() agrees)
    * Creates the bare-minimum ~/.spark directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".spark"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("SPARK_HOME", str(default_home))
    return tmp_path
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("spark_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".spark" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_spark_home_returns_default(self, profile_env):
        # SPARK_HOME points to tmp_path/.spark which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".spark" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom SPARK_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("SPARK_HOME", str(custom))
        # With Docker-aware roots, a custom SPARK_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("spark")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".spark" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("spark_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".spark" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("spark-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".spark_history",
        ...
        excluded_prefixes = [
            "default/spark-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.spark_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_spark_profiles_function(self):
        script = generate_bash_completion()
        assert "_spark_profiles" in script

    def test_zsh_completion_has_spark_function(self):
        script = generate_zsh_completion()
        assert "_spark" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultSparkHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_spark_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".spark" / "profiles"

    def test_default_spark_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_spark_home()
        assert home == tmp_path / ".spark"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (SPARK_HOME outside ~/.spark), profiles go under SPARK_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARK_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_spark_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_spark_home() returns SPARK_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARK_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (SPARK_HOME under ~/.spark), profiles root is still ~/.spark/profiles."""
        native = tmp_path / ".spark"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under SPARK_HOME."""
        from spark_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARK_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under SPARK_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARK_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARK_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".spark" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from spark_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".spark"

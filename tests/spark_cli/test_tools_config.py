"""Tests for spark_cli.tools_config platform tool persistence."""
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/automatedigital/spark/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("spark_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (spark-cli, spark-telegram, etc.) must NOT
    ...
    ``spark tools``, but spark-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "spark-cli",  # <-- the culprit
    ...
    # spark-cli must NOT survive — it's a platform default, not an MCP server
    assert "spark-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_spark_telegram():
    """Same bug for Telegram — spark-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "spark-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "spark-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "spark-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("spark_cli.tools_config._run_post_setup", lambda key: None)
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from spark_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """spark-gateway includes list should cover all messaging platforms."""
        from spark_cli.tools_config import PLATFORMS
        from core.toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["spark-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from spark_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from spark_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/automatedigital/spark/issues/6901
    """
    config = {

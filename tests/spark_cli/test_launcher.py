"""Tests for the top-level `./spark` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./spark` should use `spark_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "spark"
    called = []

    fake_main_module = types.ModuleType("spark_cli.main")

    def fake_main():
        called.append("spark_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "spark_cli.main", fake_main_module)

    fake_cli_module = types.ModuleType("cli")

    def legacy_cli_main(*args, **kwargs):
        raise AssertionError("launcher should not import core.cli as cli.main")

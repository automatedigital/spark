...
class TestReadCodexAccessToken:
    ...
    def test_valid_auth_store(self, tmp_path, monkeypatch):
        ...
        (spark_home / "auth.json").write_text(json.dumps({
            "version": 1,
            "providers": {
                "openai-codex": {
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("spark_cli.auth._read_codex_tokens", return_value={
...
class TestExpiredCodexFallback:
    ...
    def test_spark_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-spark-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic
...
class TestExplicitProviderRouting:
    ...
    def test_explicit_google_alias_uses_gemini_credentials(self):
        ...
        with (
            patch("spark_cli.auth.resolve_api_key_provider_credentials", return_value={
                "api_key": "gemini-key",
...
class TestGetTextAuxiliaryClient:
    ...
    def test_custom_endpoint_over_codex(self, monkeypatch, codex_auth_dir):
        ...
        monkeypatch.setattr("spark_cli.config.load_config", lambda: config)
        monkeypatch.setattr("spark_cli.runtime_provider.load_config", lambda: config)
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("agent.auxiliary_client.OpenAI"),
            patch("spark_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _try_codex
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_vision_auto_uses_active_provider_as_fallback(self, monkeypatch):
        """When no OpenRouter available, vision auto falls back to active provider."""
        monkeypatch.setenv("ANTHROPIC_API_KEY", "***")
        with (
            patch("agent.auxiliary_client._read_main_provider", return_value="anthropic"),
            patch("agent.auxiliary_client._read_main_model", return_value="claude-sonnet-4"),
...
def test_resolve_provider_client_supports_copilot_acp_external_process():
    ...
    with patch("agent.auxiliary_client._read_main_model", return_value="gpt-5.4-mini"), \
         ...
         patch("spark_cli.auth.resolve_external_process_provider_credentials", return_value={
...
class TestAuxiliaryMaxTokensParam:
    def test_codex_fallback_uses_max_tokens(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_codex_access_token", return_value="tok"):
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "local/custom", "openai-codex", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_to_codex_when_or_and_custom_fail(self):
...
class TestCallLlmPaymentFallback:
    ...
    def test_connection_error_triggers_fallback_when_auto(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(fallback_client, "fb-model", "local/custom")) as mock_fb:
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hello"}],
            )
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    monkeypatch.setattr("spark_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "spark_cli.auth.is_provider_explicitly_configured",
        lambda pid: False,
    )
...
class TestAsyncCallLlmFallback:
    ...
    async def test_connection_error_triggers_async_fallback(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._is_connection_error", return_value=True), \
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(fb_sync_client, "fb-model", "local/custom")) as mock_fb, \
             patch("agent.auxiliary_client._to_async_client",

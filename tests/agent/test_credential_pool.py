...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "spark" / "config.yaml"
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    monkeypatch.setenv("SPARK_HOME", str(tmp_path / "spark"))
    monkeypatch.setattr("spark_cli.auth._import_codex_cli_tokens", lambda: None, raising=False)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
...
def test_try_refresh_current_updates_only_current_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "spark_cli.auth.refresh_codex_oauth_pure",
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_spark_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("spark_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:spark_pkce", "spark_pkce"}

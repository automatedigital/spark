...
class TestCLIPersonalityNone:
    ...
    def test_none_clears_system_prompt(self):
        cli = self._make_cli()
        with patch("core.cli.commands_mixin.save_config_value", return_value=True):
    ...
    def test_none_saves_to_config(self):
        cli = self._make_cli()
        with patch("core.cli.commands_mixin.save_config_value", return_value=True) as mock_save:
            cli._handle_personality_command("/personality none")
...
class TestPersonalityDictFormat:
    ...
    def test_resolve_prompt_dict_no_tone_no_style(self):
        from core.cli import SparkCLI
        result = SparkCLI._resolve_personality_prompt({
    ...
    def test_resolve_prompt_string(self):
        from core.cli import SparkCLI
        result = SparkCLI._resolve_personality_prompt("You are helpful.")
        assert result == "You are helpful."

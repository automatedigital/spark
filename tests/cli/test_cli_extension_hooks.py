"""Tests for protected SparkCLI TUI extension hooks.
...
def _make_cli(**kwargs):
    """Create a SparkCLI with prompt_toolkit stubs (same pattern as test_cli_init)."""
    _clean_config = {
        "model": {
...
class TestExtensionHookDefaults:
    ...
    def test_build_tui_layout_children_returns_all_widgets_in_order(self):
        ...
        named_children = [child for child in children if isinstance(child, str)]
...
class TestExtensionHookSubclass:
    def test_extra_widgets_inserted_before_status_bar(self):
        ...
        named_suffix = [
            child for child in children[spacer_idx + 1 :] if isinstance(child, str)
        ]
        assert named_suffix[0] == "radio-menu"
        assert named_suffix[1] == "mini-player"
        assert named_suffix[2] == "status"

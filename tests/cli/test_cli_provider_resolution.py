...
from spark_cli.auth import AuthError
from spark_cli import main as spark_main


# ---------------------------------------------------------------------------
# Module isolation: _import_cli() wipes tools.* / cli / run_agent from
# sys.modules so it can re-import core.cli as cli fresh.  Without cleanup the wiped
...
def _restore_cli_and_tool_modules():
    ...
    prefixes = ("tools", "cli", "core.cli", "run_agent")
    original_modules = {
        name: module
...
def test_spark_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in SparkCLI.__init__")

    monkeypatch.setattr("spark_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    shell = cli.SparkCLI(model="anthropic/claude-sonnet-4", compact=True, max_turns=1)
    shell.provider = "openrouter"
    shell.api_mode = "chat_completions"
    shell.base_url = "https://openrouter.ai/api/v1"
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.SparkCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    shell = cli.SparkCLI(compact=True, max_turns=1)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.SparkCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.SparkCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "spark_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("spark_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("spark_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("spark_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("spark_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "spark_cli.models.probe_api_models",
...
def test_auto_provider_name_localhost():
    from spark_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from spark_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "spark_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("spark_cli.config.save_config", _save)

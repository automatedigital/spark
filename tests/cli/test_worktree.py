...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("spark/spark-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned spark/* and pr-* branches."""

    def test_prunes_orphaned_spark_branch(self, git_repo):
        """spark/spark-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "spark/spark-deadbeef", "HEAD"],
            cwd=str(git_repo),
            capture_output=True,
        )
        ...
        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "spark/spark-deadbeef"],
            capture_output=True,
            text=True,
            cwd=str(git_repo),
        )
        assert "spark/spark-deadbeef" in result.stdout
        ...
        assert "spark/spark-deadbeef" in orphaned
        ...
        assert "spark/spark-deadbeef" not in result.stdout

...
def _isolate(tmp_path, monkeypatch):
    """Isolate SPARK_HOME so tests don't touch real config."""
    home = tmp_path / ".spark"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal SparkCLI instance for banner testing."""
    with patch("core.cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("core.cli.get_tool_definitions", return_value=[]), \
         patch("core.cli.build_welcome_banner"):
        from core.cli import SparkCLI
        obj = SparkCLI.__new__(SparkCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["spark-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_no_warning_for_normal_context(self, cli_obj):
        ...
        with patch("core.cli.get_tool_definitions", return_value=[]), \
             patch("core.cli.build_welcome_banner"):
            cli_obj.show_banner()
    ...
    def test_compact_banner_does_not_crash_on_narrow_terminal(self, cli_obj):
        ...
        with patch("shutil.get_terminal_size", return_value=os.terminal_size((70, 40))), \
             patch("core.cli._build_compact_banner", return_value="compact banner"):
            cli_obj.show_banner()

...
- Unlimited timeout (SPARK_CRON_TIMEOUT=0)
- Backward compat: SPARK_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """SPARK_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """SPARK_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("SPARK_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """SPARK_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("SPARK_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("SPARK_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_spark_time_importable(self):
        """spark_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _spark_now
        assert callable(_spark_now)

    def test_spark_constants_importable(self):
        """spark_constants should be importable from cron context."""
        from core.spark_constants import get_spark_home
        assert callable(get_spark_home)

...
- Path resolution (absolute, relative to SPARK_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp SPARK_HOME."""
    spark_home = tmp_path / ".spark"
    spark_home.mkdir()
    (spark_home / "cron").mkdir()
    (spark_home / "cron" / "output").mkdir()
    (spark_home / "scripts").mkdir()
    monkeypatch.setenv("SPARK_HOME", str(spark_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "SPARK_DIR", spark_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", spark_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", spark_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", spark_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("SPARK_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.spark/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("SPARK_SESSION_PLATFORM") is None
        assert os.environ.get("SPARK_SESSION_CHAT_ID") is None
        assert os.environ.get("SPARK_SESSION_CHAT_NAME") is None

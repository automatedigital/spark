...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("spark_cli.config.get_config_path", return_value=config_path), \
             patch("spark_cli.config.ensure_spark_home"):
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("spark_cli.config.get_env_path", return_value=env_path), \
             patch("spark_cli.config.ensure_spark_home"):
    ...
    def test_ensure_spark_home_sets_0700(self):
        home = Path(self.tmpdir) / ".spark"
        with patch("spark_cli.config.get_spark_home", return_value=home):

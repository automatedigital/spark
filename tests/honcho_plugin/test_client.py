...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "spark"
        assert config.workspace_id == "spark"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"spark": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"spark": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"spark": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="spark-agent"
        ):
            result = config.resolve_session_name("/home/user/spark-agent/subdir")
        assert result == "spark-agent"
...
class TestResolveConfigPath:
    def test_prefers_spark_home_when_exists(self, tmp_path):
        spark_home = tmp_path / "spark"
        spark_home.mkdir()
        local_cfg = spark_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        spark_home = tmp_path / "spark"
        spark_home.mkdir()
        # No honcho.json in SPARK_HOME — also isolate ~/.spark so
    ...
    def test_falls_back_to_global_without_spark_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
...
class TestResolveActiveHost:
    def test_default_returns_spark(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("SPARK_HONCHO_HOST", None)
            os.environ.pop("SPARK_HOME", None)
            assert resolve_active_host() == "spark"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"SPARK_HONCHO_HOST": "spark.coder"}):
            assert resolve_active_host() == "spark.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SPARK_HONCHO_HOST", None)
            with patch("spark_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "spark.coder"

    def test_default_profile_returns_spark(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SPARK_HONCHO_HOST", None)
            with patch("spark_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "spark"

    def test_custom_profile_returns_spark(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SPARK_HONCHO_HOST", None)
            with patch("spark_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "spark"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SPARK_HONCHO_HOST", None)
            # Temporarily remove spark_cli.profiles to simulate import failure
            saved = sys.modules.get("spark_cli.profiles")
            sys.modules["spark_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "spark"
            finally:
                if saved is not None:
                    sys.modules["spark_cli.profiles"] = saved
                else:
                    sys.modules.pop("spark_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="spark.coder")
        assert config.host == "spark.coder"
        assert config.workspace_id == "spark"  # shared workspace
        assert config.ai_peer == "spark.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="spark")
        assert config.host == "spark"
        assert config.workspace_id == "spark"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "spark": {"aiPeer": "spark", "peerName": "alice"},
                "spark.coder": {
                    "aiPeer": "spark.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="spark.coder", config_path=config_file,
        )
        assert config.host == "spark.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "spark.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="spark.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "spark.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"spark": {"enabled": True, "aiPeer": "spark"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"spark": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestInitOnSessionStart:
    ...
    def test_host_block_overrides_root(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "initOnSessionStart": True,
            "hosts": {"spark": {"initOnSessionStart": False}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
    ...
    def test_host_block_true_overrides_root_absent(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"spark": {"initOnSessionStart": True}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.init_on_session_start is True

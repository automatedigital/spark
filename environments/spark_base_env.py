"""
SparkAgentBaseEnv -- Abstract Base Environment for Spark-Agent + Atropos

Provides the Atropos integration plumbing that all spark-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via SparkAgentLoop
...
# Ensure the spark-agent repo root is on sys.path so that imports like
...
# Load API keys from spark-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, SparkAgentLoop
...
# Import spark-agent toolset infrastructure
...
class SparkAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for spark-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of spark toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: spark, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/spark-results/ "
...
class SparkAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for spark-agent Atropos integration.
    ...
    name: Optional[str] = "spark-agent"
    env_config_cls = SparkAgentEnvConfig

    def __init__(
        self,
        config: SparkAgentEnvConfig,
        ...
        # Set terminal environment variables so spark tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY spark-agent tool (terminal, file, web,

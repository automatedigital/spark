...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Spark Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Spark Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Spark Agent",
    },
    {
...
class TerminalTestEnv(SparkAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/spark-agent/.env.
        """
        env_config = TerminalTestEnvConfig(
            ...
            tokenizer_name="AutomateDigital/q-30b-t-h45-e1",

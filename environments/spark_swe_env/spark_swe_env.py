"""
SparkSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser spark
    run-api
    python environments/spark_swe_env.py serve \\
...
class SparkSweEnv(SparkAgentBaseEnv):
    ...
    name = "spark-swe"
    env_config_cls = SparkSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[SparkSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = SparkSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepSpark-3-Llama-3-3B-Preview",
            tool_call_parser="spark",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="spark-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepSpark-3-Llama-3-3B-Preview",

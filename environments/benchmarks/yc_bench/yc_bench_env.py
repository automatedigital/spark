...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs SparkAgentLoop with terminal tool only
...
Requires: pip install spark-agent[yc-bench]
"""
...
from environments.agent_loop import SparkAgentLoop
...
class YCBenchEvalConfig(SparkAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends SparkAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(SparkAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The SparkAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs SparkAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our SparkAgentLoop.

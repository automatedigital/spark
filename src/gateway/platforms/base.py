...
def is_network_accessible(host: str) -> bool:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as _ex:
            future = _ex.submit(
                _socket.getaddrinfo, host, None, _socket.AF_UNSPEC, _socket.SOCK_STREAM
            )
        ...
        for _family, _type, _proto, _canonname, sockaddr in resolved:
            ...
            if not addr.is_loopback:
                return True
        return False
    except (_socket.gaierror, OSError, concurrent.futures.TimeoutError):
        return True
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.spark/.env manually."
)
...
# Default location: {SPARK_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_spark_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_spark_dir("cache/audio", "audio_cache")
...
DOCUMENT_CACHE_DIR = get_spark_dir("cache/documents", "document_cache")
...
class BasePlatformAdapter(ABC):
    ...
    @staticmethod
    def source_metadata(source: SessionSource, thread_id: str | None = None) -> dict[str, str] | None:
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SPARK_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SPARK_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SPARK_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        import random

        mode = os.getenv("SPARK_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("SPARK_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("SPARK_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":

...
Connects Spark Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_dir(spark_home: str) -> Path:
    path = Path(spark_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(spark_home: str, account_id: str) -> Path:
    return _account_dir(spark_home) / f"{account_id}.json"
...
def _sync_buf_path(spark_home: str, account_id: str) -> Path:
    return _account_dir(spark_home) / f"{account_id}.sync.json"


def _load_sync_buf(spark_home: str, account_id: str) -> str:
    path = _sync_buf_path(spark_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(spark_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(spark_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Spark adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(spark_home, self._account_id)
            if persisted:
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._spark_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._spark_home, self._account_id, sync_buf)
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"spark-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(self, chat_id: str, path: str, caption: str) -> str:
        ...
        if caption:
            last_message_id = f"spark-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_spark_home()))

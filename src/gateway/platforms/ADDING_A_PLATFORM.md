...
Checklist for integrating a new messaging platform into the Spark gateway.
...
"spark-your-platform": {
    ...
    "tools": _SPARK_CORE_TOOLS,
    "includes": []
},
```

And add it to the `spark-gateway` composite:

```python
"spark-gateway": {
    "includes": [..., "spark-your-platform"]
}
```
...
## 12. Status Display (`spark_cli/status.py`)
...
## 13. Gateway Setup Wizard (`spark_cli/gateway.py`)
...
| `docs/messaging/<platform>.md` | **NEW** — Full setup guide (see existing platform docs for template) |
| `docs/messaging/index.md` | Architecture diagram, toolset table, security examples, Next Steps links |
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ spark_cli/ toolsets.py \
  --include="*.py" -l | sort -u

            ...
            if user_id:
                try:
                    try:
                        ...
                        # Tests may mock a DAVE session without installing davey.
                        # Try a duck-typed call shape first, then a 3-arg fallback.
                        try:
                            decrypted = self._dave_session.decrypt(user_id, decrypted)
                        except TypeError:
                            decrypted = self._dave_session.decrypt(
                                user_id, None, decrypted
                            )
                except Exception as e:
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("SPARK_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Spark session")
        ...
        @tree.command(name="status", description="Show Spark session status")
        ...
        @tree.command(name="stop", description="Stop the running Spark agent")
        ...
        @tree.command(
            name="update", description="Update Spark Agent to the latest version"
        )
        ...
        @tree.command(
            name="thread",
            description="Create a new thread and start a Spark session in it",
        )
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Spark in the thread",
        ...
        try:
            from spark_cli.commands import discord_skill_commands
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Spark session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = (
                    starter_message or f"\U0001f9f5 Thread created by Spark: **{name}**"
                )
    ...
    async def _auto_create_thread(self, message: "DiscordMessage") -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Spark"
        if len(content) > 80:
    ...
    async def send_update_prompt(
        ...
        try:
            ...
            embed = discord.Embed(
                title="S Update Needs Your Input",
...
if DISCORD_AVAILABLE:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``spark update`` prompts.

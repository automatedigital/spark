...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("SPARK_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            def _env_float(name: str, default: float, lo: float = 1.0, hi: float = 120.0) -> float:
                try:
                    return min(max(float(os.getenv(name, str(default))), lo), hi)
                except (TypeError, ValueError):
                    return default

            request_kwargs = {
                "connection_pool_size": _env_int("SPARK_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("SPARK_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("SPARK_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("SPARK_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("SPARK_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            proxy_url = resolve_proxy_url()
            disable_fallback = (os.getenv("SPARK_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in ("1", "true", "yes", "on"))
    ...
    @staticmethod
    def _telegram_topic_kwargs(metadata: dict[str, Any] | None) -> dict[str, int | None]:
        """Build Telegram topic kwargs from platform routing metadata."""
        kwargs: dict[str, int | None] = {"message_thread_id": None}
        if not metadata:
            return kwargs
    ...
    @staticmethod
    def _telegram_retry_after(error: Exception) -> float | None:
    ...
    async def _send_telegram_request(
        ...
        kwargs: dict[str, Any],
        *,
        plain_fallback_kwargs: dict[str, Any] | None = None,
        reset_streams: list[Any] | None = None,
        context: str = "send",
    ) -> Any:
    ...
    @staticmethod
    def _telegram_id_to_str(value: Any) -> str | None:
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``spark update --gateway``
        ...
        try:
            ...
            text = f"S *Update needs your input:*\n\n{prompt}{default_hint}"
    ...
    async def _handle_callback_query(
        ...
        try:
            await query.edit_message_text(
                text=f"S Update prompt answered: *{label}*",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=None,
            )
        except Exception:

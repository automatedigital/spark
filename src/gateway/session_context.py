"""
Session-scoped context variables for the Spark gateway.

Replaces the previous ``os.environ``-based session state
(``SPARK_SESSION_PLATFORM``, ``SPARK_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["SPARK_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("SPARK_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("SPARK_SESSION_PLATFORM", "")
...
_SESSION_KEY: ContextVar[str] = ContextVar("SPARK_SESSION_KEY", default="")
...
_VAR_MAP = {
    ...
    "SPARK_SESSION_KEY": _SESSION_KEY,
}
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``SPARK_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("SPARK_SESSION_*", default)``.

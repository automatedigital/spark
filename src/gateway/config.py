...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_spark_home() / "sessions")
    ...
    def from_dict(cls, data: dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_spark_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.spark/config.yaml (primary user-facing config)
    3. ~/.spark/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_spark_home()
    gw_data: dict = {}

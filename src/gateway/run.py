...
_config_path = _spark_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["SPARK_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
            # Bridge agent.gateway_timeout → SPARK_AGENT_TIMEOUT env var.
            # Env var from .env takes precedence (already in os.environ).
            if "gateway_timeout" in _agent_cfg and "SPARK_AGENT_TIMEOUT" not in os.environ:
                os.environ["SPARK_AGENT_TIMEOUT"] = str(_agent_cfg["gateway_timeout"])
            if "gateway_timeout_warning" in _agent_cfg and "SPARK_AGENT_TIMEOUT_WARNING" not in os.environ:
                os.environ["SPARK_AGENT_TIMEOUT_WARNING"] = str(_agent_cfg["gateway_timeout_warning"])
            if "gateway_notify_interval" in _agent_cfg and "SPARK_AGENT_NOTIFY_INTERVAL" not in os.environ:
                os.environ["SPARK_AGENT_NOTIFY_INTERVAL"] = str(_agent_cfg["gateway_notify_interval"])
            if "restart_drain_timeout" in _agent_cfg and "SPARK_RESTART_DRAIN_TIMEOUT" not in os.environ:
                os.environ["SPARK_RESTART_DRAIN_TIMEOUT"] = str(_agent_cfg["restart_drain_timeout"])
        _display_cfg = _cfg.get("display", {})
        if _display_cfg and isinstance(_display_cfg, dict):
            if "busy_input_mode" in _display_cfg and "SPARK_GATEWAY_BUSY_INPUT_MODE" not in os.environ:
                os.environ["SPARK_GATEWAY_BUSY_INPUT_MODE"] = str(_display_cfg["busy_input_mode"])
        # Timezone: bridge config.yaml → SPARK_TIMEZONE env var.
        # SPARK_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "SPARK_TIMEZONE" not in os.environ:
            os.environ["SPARK_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
...
from gateway.cron_tick import start_cron_thread, start_cron_ticker
...
from gateway.session_hygiene import run_session_hygiene
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    return expand_whatsapp_auth_aliases(identifier, _spark_home)
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `spark skills config`"
                    )
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.spark/config.yaml, returning {} on any error."""
    try:
...
def _resolve_spark_bin() -> list[str] | None:
    """Resolve the Spark update command as argv parts.

    Tries in order:
    1. ``shutil.which("spark")`` — standard PATH lookup
    2. ``sys.executable -m spark_cli.main`` — fallback when Spark is running
       from a venv/module invocation and the ``spark`` shim is not on PATH
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the spark-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("spark-agent-setup") is not None
        except Exception:
            return False
    ...
    @staticmethod
    def _gateway_config_fingerprint(config: dict | None) -> str:
    ...
    def _refresh_live_config_state(self, user_config: dict | None) -> None:
    ...
    def _load_prefill_messages() -> list[dict[str, Any]]:
        ...
        file_path = os.getenv("SPARK_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _spark_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        """
        prompt = os.getenv("SPARK_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_busy_input_mode() -> str:
        ...
        mode = os.getenv("SPARK_GATEWAY_BUSY_INPUT_MODE", "").strip().lower()
        if not mode:
            try:
                import yaml as _y
                cfg_path = _spark_home / "config.yaml"
    ...
    def _load_restart_drain_timeout() -> float:
        ...
        raw = os.getenv("SPARK_RESTART_DRAIN_TIMEOUT", "").strip()
        if not raw:
            try:
                import yaml as _y
                cfg_path = _spark_home / "config.yaml"
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("SPARK_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _spark_home / "config.yaml"
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        spark_cmd = _resolve_spark_bin()
        if not spark_cmd:
            logger.error("Could not locate spark binary for detached /restart")
            return

        current_pid = os.getpid()
        cmd = " ".join(shlex.quote(part) for part in spark_cmd)
        shell_cmd = (
    ...
    async def start(self) -> bool:
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.spark/.env to allow open access, "
        ...
        # This prevents unwanted auto-resets after `spark update`,
        # `spark gateway restart`, or `/restart`.
        _clean_marker = _spark_home / ".clean_shutdown"
        ...
        if not notified and any(
            path.exists()
            for path in (
                _spark_home / ".update_pending.json",
                _spark_home / ".update_pending.claimed.json",
            )
        ):
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'spark-agent[slack]'")
                return None
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        Compatibility wrapper for gateway.authz.is_user_authorized.
        Platform map coverage includes EMAIL_ALLOWED_USERS and EMAIL_ALLOW_ALL_USERS.
    ...
    def _get_unauthorized_dm_behavior(self, platform: Platform | None) -> str:
        ...
        return get_unauthorized_dm_behavior(getattr(self, "config", None), platform)
    ...
    async def _handle_message(self, event: MessageEvent) -> str | None:
        ...
        Compatibility note: known commands such as "reasoning", "title", and
        "update" are dispatched through gateway.commands.
        """
        source = event.source

        # Mint a trace_id for this inbound message so the full processing path
        # (auth, agent loop, tool calls, reply delivery) can be correlated.
        try:
        ...
        if not await check_inbound_authorization(
            event,
            is_authorized=self._is_user_authorized,
            get_dm_behavior=self._get_unauthorized_dm_behavior,
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in ("approve", "yes"):
                ...
                response_text = raw
            if response_text:
                response_path = _spark_home / ".update_response"
                try:
        ...
        _raw_stale_timeout = float(os.getenv("SPARK_AGENT_TIMEOUT", 1800))
        ...
        if _quick_key in self._running_agents:
            ...
            from spark_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        from gateway.commands import dispatch_gateway_command

        command_handled, command_response = await dispatch_gateway_command(
            self,
            event,
            source,
            _quick_key,
        ...
        if command:
            try:
                ...
                # hyphens. See spark_cli/commands.py:_build_telegram_menu.
        ...
        if command:
            try:
                ...
                if cmd_key is not None:
                    ...
                    from gateway.commands import is_known_gateway_command

                    if not is_known_gateway_command(command):
                        logger.warning(
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the Spark venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill spark-agent-setup`"
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        history = await run_session_hygiene(
            self,
            source=source,
            session_key=session_key,
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Spark delivers cron job results "
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # Detect profile name from SPARK_HOME path
        # Profile paths look like: ~/.spark/profiles/<name>
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            (_spark_home / ".restart_notify.json").write_text(
        ...
        return "♻ Restarting gateway. If you aren't notified within 60 seconds, restart from the console with `spark gateway restart`."

    async def _handle_help_command(self, event: MessageEvent) -> str:
        """Handle /help command - list available commands."""
        from spark_cli.commands import gateway_help_lines
        lines = [
            "📖 **Spark Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `spark setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.spark/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Spark with the messaging extra, e.g. "
    ...
    async def _send_voice_reply(self, event: MessageEvent, text: str) -> None:
        ...
        try:
            ...
            audio_path = os.path.join(
                tempfile.gettempdir(), "spark_voice",
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _spark_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _run_background_task(
        ...
        try:
            ...
            max_iterations = int(os.getenv("SPARK_MAX_ITERATIONS", "90"))
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        Spawns ``spark update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``spark update`` may trigger. Marker
        ...
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "✗ /update is only available from messaging platforms. Run `spark update` from the terminal."

        if is_managed():
            return f"✗ {format_managed_message('update Spark Agent')}"
        ...
        if not spark_cmd:
            return (
                "✗ Could not locate the `spark` command. "
                "Spark is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `spark update` manually in your terminal."
            )

        pending_path = _spark_home / ".update_pending.json"
        output_path = _spark_home / ".update_output.txt"
        exit_code_path = _spark_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `spark update --gateway` detached so it survives gateway restart.
        ...
        spark_cmd_str = " ".join(shlex.quote(part) for part in spark_cmd)
        update_cmd = (
            f"PYTHONUNBUFFERED=1 {spark_cmd_str} update --gateway"
        ...
        self._schedule_update_notification_watch()
        return "S Starting Spark update… I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        """Watch ``spark update --gateway``, streaming output + forwarding prompts.
        ...
        pending_path = _spark_home / ".update_pending.json"
        claimed_path = _spark_home / ".update_pending.claimed.json"
        output_path = _spark_home / ".update_output.txt"
        exit_code_path = _spark_home / ".update_exit_code"
        prompt_path = _spark_home / ".update_prompt.json"
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ Spark update finished.")
                    else:
                        await adapter.send(chat_id, f"❌ Spark update failed (exit code {exit_code}).")
                ...
                (_spark_home / ".update_response").unlink(missing_ok=True)
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Spark update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Spark update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Spark update finished successfully."
                    else:
                        msg = "❌ Spark update failed. Check the gateway logs or run `spark update` manually for details."
    ...
    async def _send_restart_notification(self) -> None:
        ...
        notify_path = _spark_home / ".restart_notify.json"
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called spark-agent-setup that can help "
                    "users configure Spark features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called spark-agent-setup "
                                "that can help users configure Spark features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _resolved_tp
            or os.getenv("SPARK_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["SPARK_SESSION_KEY"] = session_key or ""
        ...
        # SPARK_AGENT_NOTIFY_INTERVAL env var.  Default 600s (10 min).
        # 0 = disable notifications.
        _NOTIFY_INTERVAL_RAW = float(os.getenv("SPARK_AGENT_NOTIFY_INTERVAL", 600))
        ...
        try:
            ...
            # SPARK_AGENT_TIMEOUT env var (env var takes precedence).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = float(os.getenv("SPARK_AGENT_TIMEOUT", 1800))
            ...
            if pending and pending.strip().startswith("/"):
                ...
                if _pending_cmd_word:
                    try:
                        from spark_cli.commands import resolve_command as _rc_pending
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, loop=None, interval: int = 60):
    """Compatibility wrapper for gateway.cron_tick.start_cron_ticker."""
    return start_cron_ticker(
        stop_event,
        adapters=adapters,
        loop=loop,
        interval=interval,
...
async def start_gateway(
    ...
    on_runner: Callable[["GatewayRunner"], None] | None = None,
) -> bool:
    """
    ...
    # Prevent two gateways from running under the same SPARK_HOME.
    # The PID file is scoped to SPARK_HOME, so future multi-profile
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            logger.error(
                "Another gateway instance is already running (PID %d, SPARK_HOME=%s). "
                "Use 'spark gateway restart' to replace it, or 'spark gateway stop' first.",
                existing_pid, spark_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'spark gateway restart' to replace it,\n"
                f"   or 'spark gateway stop' to kill it first.\n"
                f"   Or use 'spark gateway run --replace' to auto-replace.\n"
            )
            return False
    ...
    setup_logging(spark_home=_spark_home, mode="gateway")
    ...
    cron_stop, cron_thread = start_cron_thread(
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Spark Gateway - Multi-platform messaging")

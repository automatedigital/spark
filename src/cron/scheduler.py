...
Uses a file-based lock (~/.spark/cron/.tick.lock) so only one tick
...
# the module) fail with ModuleNotFoundError for spark_time et al.
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("SPARK_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid SPARK_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str, timeout: int | None = None) -> tuple[bool, str]:
    ...
    Args:
        script_path: Path to a Python script.  Relative paths are resolved
            against SPARK_HOME/scripts/.  Absolute and ~-prefixed paths
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _spark_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _spark_now().strftime('%H:%M:%S'), len(due_jobs))

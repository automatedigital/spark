"""
Single source of truth for provider identity in Spark Agent.
...
2. **Spark overlays** — transport type, auth patterns, aggregator flags,
...
# -- Spark overlay ----------------------------------------------------------
# Spark-specific metadata that models.dev doesn't provide.


@dataclass(frozen=True)
class SparkOverlay:
    """Spark-specific provider metadata layered on top of models.dev."""
...
SPARK_OVERLAYS: Dict[str, SparkOverlay] = {
    ...
    "openai-codex": SparkOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": SparkOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SPARK_QWEN_BASE_URL",
    ),
    "copilot-acp": SparkOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SparkOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": SparkOverlay(
    ...
    "kimi-for-coding": SparkOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": SparkOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": SparkOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": SparkOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": SparkOverlay(
        transport="openai_chat",
    ...
    "opencode": SparkOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": SparkOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""  # "models.dev", "spark", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    overlay = SPARK_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + spark extra
    ...
    if overlay is not None:
        # Spark-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,

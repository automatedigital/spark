"""
Gateway subcommand for spark CLI.

Handles: spark gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "spark-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``spark update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Spark profile are returned.
    """
    _exclude = exclude_pids or set()
    pids = [pid for pid in _get_service_pids() if pid not in _exclude]
    patterns = [
        "spark_cli.main gateway",
        "spark_cli.main --profile",
        "spark_cli.main -p",
        "spark_cli/main.py gateway",
        "spark_cli/main.py --profile",
        "spark_cli/main.py -p",
        "spark gateway",
        "gateway/run.py",
    ]
    current_home = str(get_spark_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"SPARK_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "SPARK_HOME=" in command and f"SPARK_HOME={current_home}" not in command:
            return False
        return True
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (SPARK_HOME-scoped).
...
_SERVICE_BASE = "spark-gateway"
SERVICE_DESCRIPTION = "Spark Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current SPARK_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (SPARK_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
...
def _profile_arg(spark_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when SPARK_HOME is a named profile.

    For ``~/.spark/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        spark_home: Optional explicit SPARK_HOME path. Defaults to the current
            ``get_spark_home()`` value. Should be passed when generating a
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this SPARK_HOME.

    Default ``~/.spark`` returns ``spark-gateway`` (backward compatible).
    Profile ``~/.spark/profiles/coder`` returns ``spark-gateway-coder``.
    Any other SPARK_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    spark gateway uninstall")
    print_info("    sudo spark gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Spark can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo spark gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo spark gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo spark gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Spark often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.spark`` → ``ai.spark.gateway.plist`` (backward compatible).
    Profile ``~/.spark/profiles/coder`` → ``ai.spark.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.spark.gateway-{suffix}" if suffix else "ai.spark.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.spark/spark-agent  -> /home/alice/.spark/spark-agent
      /opt/spark                 -> /opt/spark  (kept as-is)
...
def _spark_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SPARK_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_spark_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.spark                    → /home/alice/.spark
      /root/.spark/profiles/coder     → /home/alice/.spark/profiles/coder
      /opt/custom-spark               → /opt/custom-spark  (kept as-is)
    """
    current_spark = get_spark_home().resolve()
    current_default = (Path.home() / ".spark").resolve()
    target_default = Path(target_home_dir) / ".spark"

    # Default ~/.spark → remap to target user's default
    if current_spark == current_default:
        return str(target_default)

    # Profile or subdir of ~/.spark → preserve the relative structure
    try:
        relative = current_spark.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.spark) — keep as-is
        return str(current_spark)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    _venv_fallback = PROJECT_ROOT.parent / ".venv"
    venv_dir = str(detected_venv) if detected_venv else str(_venv_fallback)
    venv_bin = str(detected_venv / "bin") if detected_venv else str(_venv_fallback / "bin")
    ...
    if system:
        ...
        spark_home = _spark_home_for_target_user(home_dir)
        profile_arg = _profile_arg(spark_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m spark_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
"""
    ...
    spark_home = str(get_spark_home().resolve())
    profile_arg = _profile_arg(spark_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Spark install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}spark gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}spark gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}spark gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}spark gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}spark gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.spark.gateway-{suffix}" if suffix else "ai.spark.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Spark install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  spark gateway status             # Check status")
...
def launchd_stop():
    ...
    # `spark gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Spark install")
    else:
        print("⚠ Service definition is stale relative to the current Spark install")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Spark will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @spark:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. spark) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Spark delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Spark agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Spark will use (e.g., spark@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Spark connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: spark pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_spark_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'spark pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: spark pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'spark gateway setup'")
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Spark will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Spark will store the returned account_id/token in ~/.spark/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `spark gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_spark_home())))
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `spark pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `spark pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"SparkAgent\"")
...
def gateway_setup():
    ...
    print(color("│             S Gateway Setup                            │", Colors.MAGENTA))
    print(color("├─────────────────────────────────────────────────────────┤", Colors.MAGENTA))
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: spark gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  spark gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: spark gateway run")
                print_info("  For persistence:   tmux new -s spark 'spark gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: spark gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup spark gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: spark gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'spark gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: spark gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: spark gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s spark 'spark gateway run'")
                print()
            ...
            print()
            print("  spark gateway run                              # direct foreground")
            print("  tmux new -s spark 'spark gateway run'         # persistent via tmux")
            print("  nohup spark gateway run > ~/.spark/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: spark gateway run")
            sys.exit(0)
        else:
        ...
        if is_termux():
            ...
            print("Stop manual runs with: spark gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: spark gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: spark gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  spark gateway install")
                    print("  sudo spark gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  spark gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup spark gateway run > ~/.spark/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s spark 'spark gateway run'         # persistent via tmux")
                    print("  nohup spark gateway run > ~/.spark/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  spark gateway install  # Install as user service")
                    print("  sudo spark gateway install --system  # Install as boot-time system service")

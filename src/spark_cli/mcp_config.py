"""
MCP Server Management CLI — ``spark mcp`` subcommand.

Implements ``spark mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.spark/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from spark_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  spark mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  spark mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  spark mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_spark_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: spark mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_spark_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  spark mcp add <name> --url <endpoint>')
        _info('  spark mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'spark mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``spark mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("spark mcp serve                              Run as MCP server")
        _info("spark mcp add <name> --url <endpoint>        Add an MCP server")
        _info("spark mcp add <name> --command <cmd>         Add a stdio server")
        _info("spark mcp add <name> --preset <preset>       Add from a known preset")
        _info("spark mcp remove <name>                      Remove a server")
        _info("spark mcp list                               List servers")
        _info("spark mcp test <name>                        Test connection")
        _info("spark mcp configure <name>                   Toggle tools")
        print()

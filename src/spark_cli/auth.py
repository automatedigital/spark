...
Supports OAuth device code flows (Spark Portal, future: OpenAI Codex) and
...
# Spark Portal defaults (configure via env vars when services are available)
DEFAULT_NOUS_PORTAL_URL = os.environ.get("SPARK_PORTAL_URL", "")
DEFAULT_NOUS_INFERENCE_URL = os.environ.get("SPARK_INFERENCE_URL", "")
DEFAULT_NOUS_CLIENT_ID = "spark-cli"
...
def resolve_provider(
    ...
    *,
    ...
    if normalized in _DEPRECATED_PROVIDERS:
        raise AuthError(
            "Provider 'nous' is no longer supported. "
            "Use 'openrouter' or another configured provider.",
...
# =============================================================================
# Spark Portal — token refresh, agent key minting, model discovery
# =============================================================================
...
def fetch_nous_models(
    *,
    ...
    """Fetch available model IDs from the Spark Portal inference API."""
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Spark models — they're not reliable for agentic tool-calling
            if "spark" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware Spark Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        ...
        if not state:
            raise AuthError(
                "Spark is not logged into Spark Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("SPARK_PORTAL_BASE_URL")
        ...
        if not isinstance(access_token, str) or not access_token:
            raise AuthError(
                "No access token found for Spark Portal login.",
                provider="nous",
                relogin_required=True,
            )
...
def refresh_nous_oauth_pure(
    ...
    *,
    ...
    """Refresh Spark Portal OAuth state without mutating auth.json."""
    state: Dict[str, Any] = {
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    """Refresh Spark Portal OAuth from a state dict. Thin wrapper around refresh_nous_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_nous_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "spark-cli"),
...
def resolve_nous_runtime_credentials(
    *,
    ...
    """
    Resolve Spark Portal inference credentials for runtime use.
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError(
            "Failed to resolve a Spark Portal inference API key",
            provider="nous",
            code="server_error",
        )
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `spark status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``spark auth`` store credentials), then falls back to the legacy
...
def _nous_device_code_login(
    *,
    ...
    """Run the Spark Portal device-code flow and return full OAuth state without persisting."""
    ...
    print(f"Starting Spark login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("Your Spark Portal account does not have an active subscription.")
            print(f"  Subscribe here: {portal_url}/billing")
            print()
            print("After subscribing, run `spark model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_nous(args, pconfig: ProviderConfig) -> None:
    """Spark Portal device authorization flow."""

"""spark memory setup|status — configure memory provider plugins.
...
def _install_dependencies(provider_name: str) -> None:
    ...
    if not uv_path:
        ...
        print(f"  Then re-run: spark memory setup")
        return
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'spark memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(spark_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.spark/plugins/ and try again.\n")
        return

    # Build picker items — holographic is recommended (local, no API key)
    items = []
    holographic_idx = None
    for i, (name, desc, _) in enumerate(providers):
        label = f"{name} ★" if name == "holographic" else name
        hint = f"— {desc}" if name != "holographic" else "— local SQLite, no API key (recommended)"
        items.append((label, hint))
        if name == "holographic":
            holographic_idx = i
    items.append(("Built-in only", "— MEMORY.md / USER.md only"))

    default_idx = holographic_idx if holographic_idx is not None else len(items) - 1
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, spark_home)
        except Exception as e:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if found:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.spark/plugins/")

"""
Backup and import commands for spark CLI.

`spark backup` creates a zip archive of the entire ~/.spark/ directory
(excluding the spark-agent repo and transient files).

`spark import` restores from a backup zip, overlaying onto the current
SPARK_HOME root.
"""
...
_EXCLUDED_DIRS = {
    "spark-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to spark root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Spark home directory."""
    spark_root = get_default_spark_root()

    if not spark_root.is_dir():
        print(f"Error: Spark home directory not found at {spark_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"spark-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"spark-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_spark_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(spark_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(spark_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(spark_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside spark root
            try:
    ...
    print(f"\nRestore with: spark import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Spark backup.
    ...
    # Look for telltale files that a spark home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Spark backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.spark/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a spark dir name
        if prefix in (".spark", "spark"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_spark_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (spark_root / "config.yaml").exists()
        has_env = (spark_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Spark configuration.")
        ...
        for member in members:
            ...
            target = spark_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(spark_root.resolve())
            except ValueError:
        ...
        profiles_dir = spark_root / "profiles"
        restored_profiles = []
        ...
        # Guidance
        print()
        if not (spark_root / "spark-agent").is_dir():
            print("Note: The spark-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: spark update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  spark -p {pname} gateway install")

        print("Done. Your Spark configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and spark backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SPARK_HOME).
...
def run_quick_backup(args) -> None:
    """CLI entry point for spark backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_spark_home()}/state-snapshots/")

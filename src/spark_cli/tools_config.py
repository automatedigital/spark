"""
Unified tool configuration for Spark Agent.

`spark tools` and `spark setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.spark/config.yaml under
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from spark_cli.plugins import discover_plugins, get_plugin_toolsets
...
TOOL_CATEGORIES = {
    ...
    "browser": {
        ...
        "providers": [
            {
                "name": "Spark Subscription (Browser Use cloud)",
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_spark_home()}/spark-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in spark-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "spark-cli" (which include all _SPARK_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "spark-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `spark tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by spark tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (spark-cli, spark-telegram, etc.)
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    try:
        ...
        import core.model_tools as model_tools  # noqa: F401
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print(color("S Tool Summary", Colors.CYAN, Colors.BOLD))
        print()
        ...
        print()
        return
    print(color("S Spark Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://spark.automatedigital.ai/docs/tools", Colors.DIM))
    print()
    ...
    print()
    from core.spark_constants import display_spark_home
    print(color(f"  Tool configuration saved to {display_spark_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'spark' or gateway restart.", Colors.DIM))
    print()

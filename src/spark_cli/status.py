"""
Status command for spark CLI.

Shows the status of all Spark Agent components.
"""
...
def show_status(args):
    """Show status of all Spark Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 S Spark Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from spark_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        f"  {'Spark Portal':<12}  {check_mark(nous_logged_in)} "
        f"{'logged in' if nous_logged_in else 'not logged in (run: spark model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: spark model)'}"
    )
    ...
    # =========================================================================
    # Spark Subscription Features
    # =========================================================================
    if managed_nous_tools_enabled():
        features = get_nous_subscription_features(config)
        print()
        print(color("◆ Spark Subscription Features", Colors.CYAN, Colors.BOLD))
        if not features.nous_auth_present:
            print("  Spark Portal   ✗ not logged in")
        else:
            print("  Spark Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_nous:
                state = "active via Spark subscription"
            elif feature.active:
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: spark model)"
    ...
    if not terminal_env:
        ...
        # (spark status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if _is_termux():
        try:
            from spark_cli.gateway import find_gateway_pids
        ...
        if gateway_pids:
            ...
            print("  Start with:   spark gateway")
        ...
        from spark_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_spark_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_spark_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'spark doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'spark setup' to configure", Colors.DIM))
    print()

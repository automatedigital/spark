#!/usr/bin/env python3
"""
Spark CLI - Main entry point.

Usage:
    spark                     # Interactive chat (default)
    spark chat                # Interactive chat
    spark gateway             # Run gateway in foreground
    spark gateway start       # Start gateway as service
    spark gateway stop        # Stop gateway service
    spark gateway status      # Show gateway status
    spark gateway install     # Install gateway service
    spark gateway uninstall   # Uninstall gateway service
    spark setup               # Interactive setup wizard
    spark logout              # Clear stored authentication
    spark status              # Show status of all components
    spark cron                # Manage cron jobs
    spark cron list           # List cron jobs
    spark cron status         # Check if cron scheduler is running
    spark doctor              # Check configuration and dependencies
    spark honcho setup                    # Configure Honcho AI memory integration
    spark honcho status                   # Show Honcho config and connection status
    spark honcho sessions                 # List directory → session name mappings
    spark honcho map <name>               # Map current directory to a session name
    spark honcho peer                     # Show peer names and dialectic settings
    spark honcho peer --user NAME         # Set user peer name
    spark honcho peer --ai NAME           # Set AI peer name
    spark honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    spark honcho mode                     # Show current memory mode
    spark honcho mode [hybrid|honcho|local]  # Set memory mode
    spark honcho tokens                   # Show token budget settings
    spark honcho tokens --context N       # Set session.context() token cap
    spark honcho tokens --dialectic N     # Set dialectic result char cap
    spark honcho identity                 # Show AI peer identity representation
    spark honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    spark honcho migrate                  # Step-by-step migration guide: OpenClaw native → Spark + Honcho
    spark version             Show version
    spark update              Update to latest version
    spark uninstall           Uninstall Spark Agent
    spark acp                 Run as an ACP server for editor integration
    spark sessions browse     Interactive session picker with search

    spark claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (spark tools, spark setup, spark model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'spark {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any spark module import.
#
# Many modules cache SPARK_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SPARK_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.spark/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SPARK_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the spark root
    if profile_name is None:
        try:
            from core.spark_constants import get_default_spark_root

            active_path = get_default_spark_root() / "active_profile"
    ...
    if profile_name is not None:
        try:
            from spark_cli.profiles import resolve_profile_env

            spark_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent spark from starting
            print(
...
load_spark_dotenv(project_env=PROJECT_ROOT / ".env")

# Initialize centralized file logging early — all `spark` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from core.spark_logging import setup_logging as _setup_logging
...
def _has_any_provider_configured() -> bool:
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_spark_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Only count these if Spark has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Spark to use their tokens.
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, spark_bin
        cli_args: the original CLI arguments (everything after 'spark')
    """
    import shutil
    ...
    spark_bin = container_info["spark_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo spark {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo spark {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'spark sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Spark isn't configured yet -- no API keys or providers found."
        )
        print()
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'spark setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from spark_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["SPARK_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from spark_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Spark?")
        print()
    ...
    session_dir = get_spark_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: spark gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  spark gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with 'S Spark Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  spark gateway")
        ...
        print()
        print("  Or install as a service: spark gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'spark whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from spark_cli.setup import run_setup_wizard
...
def _do_multi_model_selection(args=None) -> None:
    """Run the provider/model picker twice to configure fast + smart models."""
    from spark_cli.config import load_config, save_config

    print()
    print("  Configure your TWO models:")
    print("  • Smart model  — powerful, used for complex / coding tasks")
    print("  • Fast model   — lightweight, used for general / simple requests")
    print()

    # ── Step 1: Smart (primary) model ───────────────────────────────────────
    print("  ── Step 1 of 2: Select your SMART model ──")
    print()
    select_provider_and_model(args=args, _inner=True, routing_slot="smart")
    smart_cfg = load_config().get("model", {})
    if isinstance(smart_cfg, str):
        smart_cfg = {"default": smart_cfg}

    smart_model_id = smart_cfg.get("default", "")
    if not smart_model_id:
        print("No smart model selected — multi-model setup cancelled.")
        return

    # ── Step 2: Fast (cheap) model ───────────────────────────────────────────
    print()
    print("  ── Step 2 of 2: Select your FAST model ──")
    print()
    select_provider_and_model(args=args, _inner=True, routing_slot="fast")
    fast_cfg = load_config().get("model", {})
    if isinstance(fast_cfg, str):
        fast_cfg = {"default": fast_cfg}

    fast_model_id = fast_cfg.get("default", "")
    if not fast_model_id:
        print("No fast model selected — multi-model setup cancelled.")
        return

    # ── Persist: smart model stays as model.default, fast → cheap_model ─────
    config = load_config()
    config["model"] = smart_cfg

    cheap_model_entry: dict = {
        "provider": fast_cfg.get("provider", ""),
        "model": fast_model_id,
    }
    if fast_cfg.get("base_url"):
        cheap_model_entry["base_url"] = fast_cfg["base_url"]
    if fast_cfg.get("api_mode"):
        cheap_model_entry["api_mode"] = fast_cfg["api_mode"]

    if not isinstance(config.get("smart_model_routing"), dict):
        config["smart_model_routing"] = {}
    config["smart_model_routing"]["enabled"] = True
    config["smart_model_routing"]["cheap_model"] = cheap_model_entry

    save_config(config)

    print()
    print("  Multi-model routing configured:")
    print(f"    Smart model : {smart_model_id}  ({smart_cfg.get('provider', '')})")
    print(f"    Fast model  : {fast_model_id}  ({fast_cfg.get('provider', '')})")
    print()
    print("  Simple requests will use the fast model automatically.")
    print("  Complex / coding requests will route to the smart model.")
...
def select_provider_and_model(
    ...
    Shared by ``cmd_model`` (``spark model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.

    When *_inner* is True the mode-selection prompt is skipped (used when
    this function is called twice from within ``_do_multi_model_selection``).

    When *routing_slot* is ``"smart"`` or ``"fast"``, provider and model
    prompts are labeled so multi-model setup stays clear.
    """
    if not _inner:
        from spark_cli.config import load_config, save_config

        _routing_cfg = load_config().get("smart_model_routing") or {}
        _current_mode_multi = bool(_routing_cfg.get("enabled"))
        _mode_choices = [
        ...
        print()
        print("  Model configuration mode:")
        _mode_idx = _prompt_provider_choice(_mode_choices, default=_mode_default)
    ...
    with model_routing_slot_selection_context(routing_slot):
        ...
        elif routing_slot == "fast":
            print("  Picking for:      FAST model (general / simple requests)")
        ...
        if selected_provider == "openrouter":
            ...
            _model_flow_ollama(config, current_model)
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.spark/.env if the active provider is not 'custom'.
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    if the user cancels.
    """
    from spark_cli.auth import get_model_routing_slot_selection

    _slot = get_model_routing_slot_selection()
    if _slot == "smart":
        _q = "Select provider (for SMART model — complex / coding):"
    elif _slot == "fast":
        _q = "Select provider (for FAST model — general / simple):"
    else:
        _q = "Select provider:"

    try:
        from spark_cli.setup import _curses_prompt_choice

        idx = _curses_prompt_choice(_q, choices, default)
        if idx >= 0:
            print()
            return idx
    except Exception:
...
def _model_flow_ollama(config, current_model=""):
    """Ollama local provider: probe running instance and pick a model.

    No API key is required.  The base URL defaults to the standard Ollama
    OpenAI-compatible endpoint (http://localhost:11434/v1) but the user can
    override it for remote or non-default-port Ollama instances.
    """
    from spark_cli.auth import _save_model_choice, deactivate_provider
    from spark_cli.config import get_env_value, load_config, save_config
    from spark_cli.models import probe_api_models

    print("Ollama local provider configuration:")
    print(f"  Default URL: {_OLLAMA_DEFAULT_URL}")
    print()

    saved_url = get_env_value("OPENAI_BASE_URL") or ""
    # Only pre-fill the saved URL if it looks like an Ollama endpoint
    current_url = saved_url if "11434" in saved_url or "ollama" in saved_url.lower() else ""

    try:
        base_url = input(
            f"Ollama base URL [{current_url or _OLLAMA_DEFAULT_URL}]: "
        ).strip()
    except (KeyboardInterrupt, EOFError):
        print("\nCancelled.")
        return

    effective_url = base_url or current_url or _OLLAMA_DEFAULT_URL

    print(f"  Probing {effective_url} ...")
    probe = probe_api_models("", effective_url)
    detected_models = probe.get("models") or []

    if not detected_models:
        print(
            "  Warning: no models found — make sure Ollama is running and has at least one model pulled."
        )
        print("  You can pull a model with: ollama pull llama3")
        print()
        try:
            model_name = input("Model name (e.g. llama3, mistral, phi3): ").strip()
        except (KeyboardInterrupt, EOFError):
            print("\nCancelled.")
            return
        if not model_name:
        ...
        from spark_cli.auth import _prompt_model_selection
        print(f"  Found {len(detected_models)} model(s):")
        selected = _prompt_model_selection(detected_models, current_model=current_model)
    ...
    print(f"Default model set to: {selected} (via Ollama at {effective_url})")

    # Auto-save to custom_providers so it appears in the /model quick-switch menu
    _save_custom_provider(effective_url, api_key="", model=selected, name="Ollama")
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `spark model` to set a model.")
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from spark_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from spark_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Spark turns to `copilot --acp`.")
    print("  Spark currently starts its own ACP subprocess for each request.")
    print("  Spark uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set SPARK_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from core.spark_constants import display_spark_home as _dhh_fn

            print(
                f"    Spark will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'spark model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Spark CLI with a provider."""
    from spark_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from spark_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from spark_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from spark_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from spark_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from spark_cli.webhook import webhook_command
...
def cmd_dump(args):
    ...
    from spark_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from spark_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from spark_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Spark home directory to a zip file."""
    if getattr(args, "quick", False):
        from spark_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from spark_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Spark backup from a zip file."""
    from spark_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Spark Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from spark_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Spark Agent."""
    _require_tty("uninstall")
    from spark_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``spark update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``spark web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (spark web will not be available)")
        )
        if fatal:
...
def _update_via_zip(args):
    """Update Spark Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/automatedigital/spark/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="spark-update-")
        zip_path = os.path.join(tmp_dir, f"spark-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to spark-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"spark-agent-{branch}")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Spark behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes spark completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Spark couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Spark couldn't drop the saved stash entry."
            )
...
# =========================================================================
# Fork detection and upstream management for `spark update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/automatedigital/spark.git",
    "git@github.com:automatedigital/spark.git",
    "https://github.com/automatedigital/spark",
    "git@github.com:automatedigital/spark",
}
OFFICIAL_REPO_URL = "https://github.com/automatedigital/spark.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_spark_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Spark repository.")
        print("  This means you may miss updates from automatedigital/spark.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/automatedigital/spark.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/automatedigital/spark.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``spark update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from core.spark_constants import get_default_spark_root

    default_home = get_default_spark_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "spark-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Spark Agent")
        return
    ...
    try:
        ...
        # the old bytecode (e.g. get_spark_home added to spark_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload spark_constants so that any lazy
        ...
        if needs_migration:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: spark config migrate")
            else:
                print()
        ...
        # When running as ``spark update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart spark-gateway`` kills everything in the
        ...
        print("  spark model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``spark -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `spark profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_spark_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → spark -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.spark)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  spark profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     spark -p {name} chat")
                else:
        ...
        from spark_cli.profiles import profile_exists
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install spark-agent[web]")
        sys.exit(1)

    if not _build_web_ui(Path(__file__).parent / "web", fatal=True):
        sys.exit(1)
...
def cmd_completion(args):
    ...
    from spark_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    """Main entry point for spark CLI."""
    parser = argparse.ArgumentParser(
        prog="spark",
        description="Spark Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    spark                        Start interactive chat
    spark chat -q "Hello"        Single query mode
    spark -c                     Resume the most recent session
    spark -c "my project"        Resume a session by name (latest in lineage)
    spark --resume <session_id>  Resume a specific session by ID
    spark setup                  Run setup wizard
    spark logout                 Clear stored authentication
    spark auth add <provider>    Add a pooled credential
    spark auth list              List pooled credentials
    spark auth remove <p> <t>    Remove pooled credential by index, id, or label
    spark auth reset <provider>  Clear exhaustion status for a provider
    spark model                  Select default model
    spark config                 View configuration
    spark config edit            Edit config in $EDITOR
    spark config set model gpt-4 Set a config value
    spark gateway                Run messaging gateway
    spark -s spark-agent-dev,github-auth
    spark -w                     Start in isolated git worktree
    spark gateway install        Install gateway background service
    spark sessions list          List past sessions
    spark sessions browse        Interactive session picker
    spark sessions rename ID T   Rename/title a session
    spark logs                   View agent.log (last 50 lines)
    spark logs -f                Follow agent.log in real time
    spark logs errors            View errors.log
    spark logs --since 1h        Lines from the last hour
    spark debug share             Upload debug report for support
    spark update                 Update to latest version
...
""",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Spark Agent",
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Spark CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: spark-cli)"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Spark Agent components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Spark Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Spark setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Spark Agent. Use 'spark debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    spark debug share              Upload debug report and print URL
    spark debug share --lines 500  Include more log lines
    spark debug share --expire 30  Keep paste for 30 days
    spark debug share --local      Print report locally (no upload)
""",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Spark home directory to a zip file",
        description="Create a zip archive of your entire Spark configuration, "
        "skills, sessions, and data (excludes the spark-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/spark-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Spark backup from a zip file",
        description="Extract a previously created Spark backup into your "
        "Spark home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Spark Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from spark_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from spark_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from spark_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/spark-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    def cmd_plugins(args):
        from spark_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from spark_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'spark tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # spark tools disable <name...> [--platform cli]
    ...
    # spark tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            _require_tty("tools")
            from spark_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Spark as an MCP server",
        description=(
            "Manage MCP server connections and run Spark as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'spark mcp add' to connect to a new server, or\n"
            "'spark mcp serve' to expose Spark conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Spark as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from spark_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch spark --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil

            spark_bin = shutil.which("spark")
            if spark_bin:
                os.execvp(spark_bin, ["spark", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "spark_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Spark",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Spark",
    ...
    def cmd_claw(args):
        from spark_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Spark Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Spark Agent",
        description="Remove Spark Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Spark Agent as an ACP (Agent Client Protocol) server",
        description="Start Spark Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Spark Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Spark instances",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Spark log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    spark logs                    Show last 50 lines of agent.log
    spark logs -f                 Follow agent.log in real time
    spark logs errors             Show last 50 lines of errors.log
    spark logs gateway -n 100     Show last 100 lines of gateway.log
    spark logs --level WARNING    Only show WARNING and above
    spark logs --session abc123   Filter by session ID
    spark logs --component tools  Only show tool-related lines
    spark logs --since 1h         Lines from the last hour
    spark logs --since 30m -f     Follow, starting from 30 min ago
    spark logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``spark -c Pokemon Agent Dev`` → ``spark -c 'Pokemon Agent Dev'``
    ...
    from spark_cli.config import get_container_exec_info

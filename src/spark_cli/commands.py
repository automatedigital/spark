"""Slash command definitions and autocomplete for the Spark CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Spark config/state", "Session",
    ...
    CommandDef("learnings", "Review what Dream learned and confirm flagged memory removals",
               "Tools & Skills",
               aliases=("learned",), web_available=True),
    CommandDef("curator", "Manage background skill maintenance (consolidate, archive)",
               "Tools & Skills",
               args_hint="[status|pause|resume|run]",
    ...
    CommandDef("connectors", "Connect external platforms (Google Workspace, …) for the agent to use",
               "Tools & Skills", cli_only=True, web_available=True,
               aliases=("connect", "disconnect"),
               args_hint="[connect|disconnect|status] <id>",
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``spark skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /spark handler.

    Maps both canonical names and aliases so /spark bg do stuff works
    the same as /spark background do stuff.
    """
    overrides = _resolve_config_gates()

"""spark webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.spark/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: spark gateway run
"""
...
def webhook_command(args):
    """Entry point for 'spark webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: spark webhook {subscribe|list|remove|test}")
        print("Run 'spark webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (spark gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: spark webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (spark gateway run)")

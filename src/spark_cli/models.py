...
Add, remove, or reorder entries here — both `spark setup` and
`spark` provider-selection will pick up the change automatically.
"""
...
#   tui_desc    — longer description for the `spark model` interactive picker
# ---------------------------------------------------------------------------
...
class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str  # detailed description for `spark model` TUI
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry(
        "kimi-coding", "Kimi / Moonshot", "Kimi / Moonshot (Moonshot AI direct API)"
    ),
    ProviderEntry(
        "kimi-coding-cn",
        "Kimi / Moonshot (China)",
    ...
    ProviderEntry(
        "ollama",
        "Ollama (local)",
        "Ollama (local models — no API key required, runs on your machine)",
    ),
]
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``spark model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``spark auth add openai-codex`` without
    ``spark model``).
    """
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout, **urllib_request_kwargs()) as resp:
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``spark model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from spark_cli.auth import get_auth_status, has_usable_secret
...
def _get_ollama_base_url() -> str:
    ...
    Priority: OLLAMA_BASE_URL env var → model.base_url (when provider=ollama)
    ...
    try:
        ...
        config = load_config()
        model_cfg = config.get("model", {})
        if isinstance(model_cfg, dict) and model_cfg.get("provider") == "ollama":
            cfg_url = str(model_cfg.get("base_url", "")).strip()
            if cfg_url:
                return cfg_url
    except Exception:
        pass
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Spark' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``spark_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Spark should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from spark_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(
    ...
    Tries live API endpoints for providers that support them (for example Codex),
    ...
    if normalized == "ollama":
        ...
        if ollama_url:
            live = fetch_api_models("", ollama_url)
            if live:
                return live
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from spark_cli.copilot_auth import copilot_request_headers
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from core.spark_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Spark will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )

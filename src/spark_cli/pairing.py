...
Usage:
    spark pairing list              # Show all pending + approved users
    spark pairing approve <platform> <code>  # Approve a pairing code
    spark pairing revoke <platform> <user_id> # Revoke user access
    spark pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle spark pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: spark pairing {list|approve|revoke|clear-pending}")
        print("Run 'spark pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print("  Run 'spark pairing list' to see pending codes.\n")

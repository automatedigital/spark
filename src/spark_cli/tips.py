...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or spark -c.",
    ...
    "Ctrl+Z suspends Spark to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "spark -c resumes your most recent CLI session. spark -c \"project name\" resumes by title.",
    "spark -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "spark -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "spark chat -t web,terminal enables only specific toolsets for a focused session.",
    "spark chat -s github-pr-workflow preloads a skill at launch.",
    "spark chat -q \"query\" runs a single non-interactive query and exits.",
    "spark chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "spark chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "spark --yolo bypasses all dangerous command approval prompts for the entire session.",
    "spark chat --source telegram tags the session for filtering in spark sessions list.",
    "spark -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "spark doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "spark dump outputs a compact setup summary — great for bug reports.",
    "spark config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "spark config edit opens config.yaml in your default editor.",
    "spark config check scans for missing or stale configuration options.",
    "spark sessions browse opens an interactive session picker with search.",
    "spark sessions stats shows session counts by platform and database size.",
    "spark sessions prune --older-than 30 cleans up old sessions.",
    "spark skills search react --source skills-sh searches the skills.sh public directory.",
    "spark skills check scans installed hub skills for upstream updates.",
    "spark skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "spark skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "spark mcp add github --command npx adds MCP servers from the command line.",
    "spark mcp serve runs Spark itself as an MCP server for other agents.",
    "spark auth add lets you add multiple API keys for credential pool rotation.",
    "spark completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "spark logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "spark backup creates a zip backup of your entire Spark home directory.",
    "spark profile create coder creates an isolated profile that becomes its own command.",
    "spark profile create work --clone copies your current config and keys to a new profile.",
    "spark update syncs new bundled skills to ALL profiles automatically.",
    "spark gateway install sets up Spark as a system service (systemd/launchd).",
    "spark memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "spark webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as spark -w).",
    ...
    "SPARK_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Spark tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'spark profile create coder' creates the 'coder' command.",
    "spark profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Spark prints a resume command with session ID and stats.",
    "spark sessions export backup.jsonl exports all sessions for backup or analysis.",
    "spark -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "spark skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .spark/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: spark cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Spark runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "spark gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Spark via a local macOS server.",
    ...
    "Context files (.spark.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.spark/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Spark loads project context from .spark.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — spark tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.spark/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "core.batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "spark chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "spark plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "spark acp runs Spark as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "BOOT.md at ~/.spark/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.spark/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Spark your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.spark/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Spark auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.spark/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SPARK_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "SPARK_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SPARK_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.spark/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'spark profile create ls' would be rejected.",
    "spark profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "spark claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SPARK_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via spark tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.spark/.env.",
    "Skills from trusted repos (AutomateDigital) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.spark/skills/.hub/quarantine/ holds skills pending security review.",
]

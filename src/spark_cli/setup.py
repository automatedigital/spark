"""
Interactive setup wizard for Spark Agent.

Modular wizard with independently-runnable sections:
  1. Model & Provider — choose your AI provider and model
  2. Terminal Backend — where your agent runs commands
  3. Agent Settings — iterations, compression, session reset
  4. Messaging Platforms — connect Telegram, Discord, etc.; optional OpenAI-compatible HTTP API
  5. Tools — configure TTS, web search, image generation, etc.

Config files are stored in ~/.spark/ for easy access.
"""
...
# display_spark_home imported lazily at call sites (stale-module safety during spark update)
...
from spark_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("S Spark Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Spark using environment variables or config commands:")
    print_info("  spark config set model.provider custom")
    print_info("  spark config set model.base_url http://localhost:8080/v1")
    print_info("  spark config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'spark setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'spark setup')")


def _print_setup_summary(config: dict, spark_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'spark setup' to configure"))
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (Spark subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (Spark Portal Browser Use)", True, None))
    ...
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (Spark subscription)", True, None))
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via Spark subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'spark setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (Spark subscription)", True, None))
    elif config.get("terminal", {}).get("backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'spark setup terminal'"))
    elif managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via Spark subscription)", True, None))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'spark setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {spark_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('spark setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('spark setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('spark setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('spark setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('spark setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('spark config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('spark config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('spark config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('spark', Colors.GREEN)}              Start chatting")
    print(f"   {color('spark gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('spark doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `spark tools` and `spark setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``spark model``)
    ...
    provider added to ``spark model`` is automatically available here.
    ...
    # Delegate to the shared spark model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: spark model")
    ...
    if "smart_model_routing" in _refreshed:
        config["smart_model_routing"] = _refreshed["smart_model_routing"]
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Spark can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Spark can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'spark setup' or configure AUXILIARY_VISION_* settings")
    ...
    if selected_provider == "nous" and nous_subscription_selected:
        ...
        if "tts" in changed_defaults:
            print_success("TTS provider set to: OpenAI TTS via your Spark subscription")
        else:
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("Spark Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "nous-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Spark Portal gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.spark/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'spark setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Spark runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Spark via Telegram/Discord, this is where")
        print_info(
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Spark Portal gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("SPARK_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `spark setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    print_info(f"   Guide: {_DOCS_BASE}/configuration")
    print()

    # ── Max Iterations ──
    current_max = get_env_value("SPARK_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("SPARK_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Spark delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://spark.automatedigital.ai/docs/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Spark delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Spark delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'spark whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'spark whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from spark_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from spark_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from spark_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from spark_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from spark_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from spark_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from spark_cli.gateway import _setup_wecom as _gateway_setup_wecom
...
def _setup_bluebubbles():
    ...
    print_info("Connects Spark to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from spark_cli.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from spark_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print_info("   https://spark.automatedigital.ai/docs/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  spark config edit")
...
def setup_http_api(_config: dict) -> None:
    """Offer to enable the OpenAI-compatible HTTP API (Bearer token; default host/port in .env)."""
    from spark_cli.auth import has_usable_secret
    from spark_cli.config import (
        HTTP_API_DEFAULT_HOST,
        HTTP_API_DEFAULT_PORT,
        is_managed,
        managed_error,
    )

    if is_managed():
        managed_error("configure HTTP API")
        return

    enabled_raw = (get_env_value("API_SERVER_ENABLED") or "").strip().lower()
    if enabled_raw in ("true", "1", "yes"):
        existing_key = (get_env_value("API_SERVER_KEY") or "").strip()
        if existing_key and has_usable_secret(existing_key, min_length=8):
            print_info(
                "OpenAI-compatible HTTP API already enabled in ~/.spark/.env (skipped)."
            )
            return

    print_header("HTTP API (OpenAI-compatible)")
    print_info(
        "Expose Spark at /v1/* for Open WebUI, scripts, and other OpenAI-style clients."
    )
    print_info(
        f"Defaults: host {HTTP_API_DEFAULT_HOST}, port {HTTP_API_DEFAULT_PORT} (written to ~/.spark/.env)."
    )
    print()

    if not prompt_yes_no("Enable the HTTP API?", False):
        print_info("Skipped. Uncomment or add API_SERVER_* in ~/.spark/.env later.")
        return

    while True:
        pwd = prompt(
            "  API password (Bearer token — clients send Authorization: Bearer <token>)",
            password=True,
        ).strip()
        if not pwd:
            print_warning("Empty password — HTTP API not enabled.")
            return
        if has_usable_secret(pwd, min_length=8):
            break
        print_warning(
            "  Use at least 8 characters; avoid common placeholders (e.g. changeme, ***)."
        )

    try:
        save_http_api_env_block(pwd)
    except ValueError as exc:
        print_error(str(exc))
        return

    print_success("HTTP API enabled in ~/.spark/.env")
    print_info(
        f"  Run `spark gateway` then point clients at "
        f"http://{HTTP_API_DEFAULT_HOST}:{HTTP_API_DEFAULT_PORT}/v1"
    )
    print_info("  Use the same token as the client's OpenAI API key setting.")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     spark config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: spark gateway install")
            else:
                print_info("  You can install later: spark gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo spark gateway install --system")
                print_info("  Or run in foreground:  spark gateway")
        else:
            from core.spark_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   spark gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   spark gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `spark setup tools` and `spark tools` use the same flow:
...
# =============================================================================
# Section 6: Memory Provider
# =============================================================================


def setup_memory(config: dict):
    """Configure memory provider — defaults to holographic (local SQLite, no API key)."""
    from spark_cli.memory_setup import cmd_setup
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from spark_cli.config
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_spark.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_spark migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_spark", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Spark.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Spark to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Spark at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Spark at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Spark at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Spark at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Spark equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Spark config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Spark.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Spark's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(spark_home: Path) -> bool:
    ...
    print_info("Spark can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: spark claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=spark_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: spark claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Spark configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Spark config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Spark (use spark claw migrate --overwrite to force).")
    if skipped:
...
SETUP_SECTIONS = [
    ...
    ("memory", "Memory Provider", setup_memory),
]
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      spark setup           — full or quick (auto-detected)
      spark setup model     — just model/provider
      spark setup tts       — just text-to-speech
      spark setup terminal  — just terminal backend
      spark setup gateway   — just messaging platforms
      spark setup tools     — just tool configuration
      spark setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     S Spark Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             S Spark Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Spark Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Spark configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, spark_home)
            return
        elif choice == 1:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, spark_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(spark_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, spark_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {spark_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'spark config edit'")
...
def _offer_launch_chat():
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        print()
        print_info("Run 'spark' to start chatting.")
        return

    print()
    if not prompt_yes_no("Launch spark chat now?", True):
        return

    chat_argv = _resolve_spark_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Spark automatically. Run 'spark chat' manually.")
        return

    # The setup wizard may have been launched with `< /dev/tty` (e.g. from the
    # install script running inside `curl | bash`).  In that case fd 0 is the
    # magic /dev/tty character device, which macOS kqueue cannot poll — causing
    # prompt_toolkit to crash with OSError EINVAL immediately on startup.
    #
    # Fix: find the actual PTY slave device via stdout/stderr (those fds were
    # never redirected and always point to the real pty, e.g. /dev/ttys001),
    # then dup it onto stdin before exec so prompt_toolkit gets a pollable fd.
    try:
        real_tty = None
        for _fd in (sys.stdout.fileno(), sys.stderr.fileno()):
            try:
                _path = os.ttyname(_fd)
                if _path and _path != "/dev/tty":
                    real_tty = _path
                    break
            except OSError:
                pass
...
def _run_first_time_quick_setup(config: dict, spark_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'spark setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    spark setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  spark setup gateway")
    print()
...
def _run_quick_setup(config: dict, spark_home):
    ...
    # Always check if a model/provider is configured — if not, run provider
    # setup first regardless of other missing items.  This guards against the
    # case where is_existing=True (env var present) but the model was never
    # actually chosen.
    _model_cfg = config.get("model")
    _has_model = bool(
        (isinstance(_model_cfg, dict) and _model_cfg.get("default"))
        or (isinstance(_model_cfg, str) and _model_cfg.strip())
    )
    if not _has_model:
        print_info("No model configured yet — let's pick one first.")
        print()
        setup_model_provider(config, quick=True)
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'spark setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Spark to messaging apps to chat from anywhere.")

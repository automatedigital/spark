"""Helpers for Spark subscription managed-tool capabilities."""
...
from spark_cli.config import get_env_value, load_config
...
def get_nous_subscription_features(
    ...
    provider_is_spark = str(model_cfg.get("provider") or "").strip().lower() == "nous"
    ...
    subscribed = provider_is_spark or nous_auth_present
    ...
    features = {
        ...
        "image_gen": NousFeatureState(
            ...
            current_provider="FAL" if direct_fal else ("Spark Subscription" if image_managed else ""),
    ...
    return NousSubscriptionFeatures(
        ...
        provider_is_spark=provider_is_spark,
        features=features,
    )
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        "Spark subscription enables managed web tools, image generation, OpenAI TTS, and browser automation by default.",
        "Those managed tools bill to your Spark subscription. Modal execution is optional and can bill to your subscription too.",
        "Change these later with: spark setup tools, spark setup terminal, or spark status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Spark Portal defaults shared by `spark setup` and `spark model`."""
    if not managed_nous_tools_enabled():
        return set()

    features = get_nous_subscription_features(config)
    if not features.provider_is_spark:
        return set()

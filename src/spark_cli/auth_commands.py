...
def auth_add_command(args) -> None:
    ...
    if provider in {"nous", "nous-research"}:
        raise SystemExit(
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = (
                AUTH_TYPE_OAUTH
                if provider in {"anthropic", "openai-codex", "qwen-oauth"}
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_spark_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:spark_pkce",
    ...
    raise SystemExit(
        f"`spark auth add {provider}` is not implemented for auth type {requested_type} yet."
    )
        ...
        providers = sorted(
            {
                *(
                    ...
                    if pid not in _DEPRECATED_PROVIDERS
                ),
                "openrouter",
                *list_custom_pool_providers(),
            }
...
def auth_remove_command(args) -> None:
    ...
    # If this was a singleton-seeded credential (OAuth device_code, spark_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "spark_pkce" and provider == "anthropic":
        from core.spark_constants import get_spark_home

        oauth_file = get_spark_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Spark Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from spark_cli.auth import suppress_credential_source
        ...
        print("Run `spark auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `spark auth` is called bare."""

...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from spark_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if configured_mode and _provider_supports_explicit_api_mode(
            ...
            from spark_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    if isinstance(cfg_provider, str) and cfg_provider.strip():
        return _normalize_deprecated_provider(cfg_provider)

    # Prefer the persisted config selection over any stale shell/.env
    # provider override so chat uses the endpoint the user last saved.
    env_provider = os.getenv("SPARK_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return _normalize_deprecated_provider(env_provider)

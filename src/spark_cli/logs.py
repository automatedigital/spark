"""``spark logs`` — view and filter Spark log files.
...
Usage examples::
    ...
    spark logs errors             # last 50 lines of errors.log
    spark logs gateway -n 100    # last 100 lines of gateway.log
    spark logs --level WARNING    # only WARNING+ lines
    spark logs --session abc123   # filter by session ID substring
    spark logs --component tools  # only tool-related lines
    spark logs --since 1h         # lines from the last hour
    spark logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if follow:
        print(f"--- {display_spark_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_spark_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_spark_home()}/logs/")
        return

    print(f"Log files in {display_spark_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'spark chat' to generate logs)")

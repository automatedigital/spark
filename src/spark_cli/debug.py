"""``spark debug`` — debug tools for Spark Agent.

Currently supports:
    spark debug share    Upload debug report (system info + logs) to a
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "spark-agent/debug-share",
        },
    )
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----SparkDebugBoundary9f3c"
...
def _read_log_tail(log_name: str, num_lines: int) -> str:
    ...
    from spark_cli.logs import _read_last_n_lines
...
def _capture_dump() -> str:
    """Run ``spark dump`` and return its stdout as a string."""
...
def collect_debug_report(*, log_lines: int = 200, dump_text: str = "") -> str:
    ...
    dump_text
        Pre-captured dump output.  If empty, ``spark dump`` is run
        internally.
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: spark debug share [--lines N] [--expire N] [--local]")
        print()
        print("Commands:")

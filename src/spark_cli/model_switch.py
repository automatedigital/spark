...
- ``spark_cli.providers``        -- canonical provider identity + overlays
- ``spark_cli.model_normalize``  -- per-provider name formatting
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'spark model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'spark doctor' — config issues detected:"
    ...
    spark_warn = _check_spark_model_warning(new_model)
    if spark_warn:
        warnings.append(spark_warn)
...
def list_authenticated_providers(
    ...
    # Build curated model lists keyed by spark provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # --- 1. Check Spark-mapped providers ---
    for spark_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(spark_id)
        ...
        model_ids = curated.get(spark_id, [])
        total = len(model_ids)
    ...
    # while _PROVIDER_MODELS and config.yaml use Spark IDs ("copilot").
    _mdev_to_spark = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in SPARK_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve Spark slug — e.g. "github-copilot" → "copilot"
        spark_slug = _mdev_to_spark.get(pid, pid)
        if spark_slug in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from spark_cli.auth import _load_auth_store
                ...
                if store and (
                    ...
                    or spark_slug in providers_store
                    or pid in pool_store
                    or spark_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(spark_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", spark_slug, exc)
        ...
        # configured.
        if not has_creds and spark_slug == "anthropic":
            try:
                ...
                if (spark_creds and spark_creds.get("accessToken")) or (
        ...
        # Use curated list — look up by Spark slug, fall back to overlay key
        model_ids = curated.get(spark_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]
        ...
        results.append(
            {
                "slug": spark_slug,
                "name": get_label(spark_slug),
                "is_current": spark_slug == current_provider or pid == current_provider,
                "is_user_defined": False,
                "models": top,
                "total_models": total,
                "source": "spark",
        ...
        seen_slugs.add(pid)
        seen_slugs.add(spark_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or SPARK_OVERLAYS (keeps /model in sync
    # with `spark model`).
    try:
        from spark_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []

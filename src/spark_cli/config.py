"""
Configuration management for Spark Agent.

Config files are stored in ~/.spark/ for easy access:
- ~/.spark/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.spark/.env         - API keys and secrets

This module provides:
- spark config          - Show current configuration
- spark config edit     - Open config in editor
- spark config set      - Set a specific value
- spark config wizard   - Re-run setup wizard
"""
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_spark_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Spark is running in package-manager-managed mode.

    Two signals: the SPARK_MANAGED env var (set by the systemd service),
    or a .managed marker file in SPARK_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade spark-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "spark update"


def format_managed_message(action: str = "modify this Spark installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SPARK_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Spark installation is managed by NixOS "
            f"(SPARK_MANAGED={env_hint}).\n"
            "Edit services.spark-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Spark installation is managed by Homebrew "
            f"(SPARK_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade spark-agent"
        )

    return (
        f"Cannot {action}: this Spark installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Spark."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container, or SPARK_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SPARK_DEV") == "1":
        return None
    ...
    container_mode_file = get_spark_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "spark-agent")
    exec_user = info.get("exec_user", "spark")
    spark_bin = info.get("spark_bin", "/data/current-package/bin/spark")
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the spark group can
    share state with the gateway service.

    The mode can be overridden via the SPARK_HOME_MODE environment variable
    (e.g. SPARK_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SPARK_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SPARK_HOME_MODE", "").strip()
...
def ensure_spark_home():
    """Ensure ~/.spark directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_spark_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_spark_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"SPARK_HOME {home} does not exist. Run 'sudo nixos-rebuild switch' first."
        )
    for subdir in ("cron", "sessions", "logs", "memories"):
        d = home / subdir
        if not d.is_dir():
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Spark
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",  # auto | openrouter | openai-codex | custom
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.spark/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        # Set to a provider name to activate: "holographic" (default, local SQLite),
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "S *Spark Agent*" header.
    ...
    # Logging — controls file logging to ~/.spark/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @spark:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SPARK_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'spark-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # SPARK_TOOL_PROGRESS and SPARK_TOOL_PROGRESS_MODE are deprecated —
...
def validate_config_structure(
        ...
        try:
            ...
            return [
                ConfigIssue(
                    ...
                    "Run 'spark setup' to create a valid config",
                )
            ]
    ...
    if cp and not model_cfg:
        issues.append(
            ConfigIssue(
                "warning",
                "custom_providers defined but no 'model' section — Spark won't know which provider to use",
                "Add a model section:\n"
                "  model:\n"
                "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'spark doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SPARK_TOOL_PROGRESS")
            old_mode = get_env_value("SPARK_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append(
                    "display.tool_progress=off (from SPARK_TOOL_PROGRESS=false)"
                )
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(
                    f"display.tool_progress={old_mode.lower()} (from SPARK_TOOL_PROGRESS_MODE)"
                )
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SPARK_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(
                    f"timezone={old_tz.strip()} (from SPARK_TIMEZONE)"
                )
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: spark config set <key> <value>")
    ...
    # metadata.spark.config in their SKILL.md frontmatter.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Spark env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.spark/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.spark/.env."""
    if is_managed():
...
HTTP_API_DEFAULT_HOST = "127.0.0.1"
HTTP_API_DEFAULT_PORT = "8642"


def _is_api_server_env_line(line: str) -> bool:
    """True if line assigns API_SERVER_ENABLED|KEY|HOST|PORT (commented or not)."""
    s = line.strip()
    if s.startswith("#"):
        s = s[1:].strip()
    return s.startswith(
...
def save_http_api_env_block(api_key: str) -> None:
    """Write API server settings to ~/.spark/.env, replacing any prior API_SERVER_* lines.

    Sets API_SERVER_ENABLED=true, API_SERVER_KEY, API_SERVER_HOST (127.0.0.1),
    and API_SERVER_PORT (8642). Removes previous assignments whether commented or not.
    """
    if is_managed():
    ...
    if not has_usable_secret(api_key, min_length=8):
        raise ValueError(
            "API key must be at least 8 characters and not a common placeholder"
        )
    ...
    kept = [ln for ln in lines if not _is_api_server_env_line(ln.rstrip("\r\n"))]
    block = [
        "API_SERVER_ENABLED=true\n",
        f"API_SERVER_KEY={api_key}\n",
        f"API_SERVER_HOST={HTTP_API_DEFAULT_HOST}\n",
        f"API_SERVER_PORT={HTTP_API_DEFAULT_PORT}\n",
    ]
    ...
    os.environ["API_SERVER_HOST"] = HTTP_API_DEFAULT_HOST
    os.environ["API_SERVER_PORT"] = HTTP_API_DEFAULT_PORT
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.spark/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.spark/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Spark — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Spark vars that are no longer in .env
...
def show_config():
    ...
    print(
        color("│              S Spark Configuration                    │", Colors.CYAN)
    )
    print(
        color(
            "└─────────────────────────────────────────────────────────┘", Colors.CYAN
    ...
    print(color("  spark config edit     # Edit config file", Colors.DIM))
    print(color("  spark config set <key> <value>", Colors.DIM))
    print(color("  spark setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: spark config set <key> <value>")
            print()
            print("Examples:")
            print("  spark config set model anthropic/claude-sonnet-4")
            print("  spark config set terminal.backend docker")
            print("  spark config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'spark config migrate' to add them")
        ...
        print("  spark config           Show current configuration")
        print("  spark config edit      Open config in editor")
        print("  spark config set <key> <value>   Set a config value")
        print("  spark config check     Check for missing/outdated config")
        print("  spark config migrate   Update config with new options")
        print("  spark config path      Show config file path")
        print("  spark config env-path  Show .env file path")
        sys.exit(1)

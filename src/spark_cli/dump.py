"""
Dump command for spark CLI.

Outputs a compact, plain-text summary of the user's Spark setup
...
def _count_skills(spark_home: Path) -> int:
...
def _cron_summary(spark_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = spark_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"spark_home:      {display_spark_home()}")
    ...
    toolsets = config.get("toolsets", ["spark-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(spark_home)}")
    lines.append(f"  skills:             {_count_skills(spark_home)}")

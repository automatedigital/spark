"""
Profile management for multiple isolated Spark instances.

Each profile is a fully independent SPARK_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.spark/profiles/<name>/`` by default.

The "default" profile is ``~/.spark`` itself — backward compatible,
zero migration needed.

Usage::

    spark profile create coder          # fresh profile + bundled skills
    spark profile create coder --clone  # also copy config, .env, SOUL.md
    spark profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    spark -p coder chat                 # or via flag
    spark profile use coder             # set as sticky default
    spark profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See spark_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.spark) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "spark-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "spark", "default", "test", "tmp", "root", "sudo",
})

# Spark subcommands that cannot be used as profile names/aliases
_SPARK_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the spark root, NOT to the current SPARK_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SPARK_HOME points outside
    ``~/.spark``, profiles live under ``SPARK_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_spark_home() / "profiles"


def _get_default_spark_home() -> Path:
    """Return the default (pre-profile) SPARK_HOME path.

    In standard deployments this is ``~/.spark``.
    In Docker/custom deployments where SPARK_HOME is outside ``~/.spark``
    (e.g. ``/opt/data``), returns SPARK_HOME directly.
    """
    from core.spark_constants import get_default_spark_root
    return get_default_spark_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_spark_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.spark
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SPARK_HOME directory."""
    if name == "default":
        return _get_default_spark_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if name in _SPARK_SUBCOMMANDS:
        return f"'{name}' conflicts with a spark subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_spark_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.spark)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SPARK_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.spark).\n"
            "To remove everything, use: spark uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SPARK_HOME so _profile_suffix resolves correctly
    ...
    try:
        ...
        if old_home is not None:
            os.environ["SPARK_HOME"] = old_home
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.spark/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: spark profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SPARK_HOME.

    Returns ``"default"`` if SPARK_HOME is not set or points to ``~/.spark``.
    Returns the profile name if SPARK_HOME points into ``~/.spark/profiles/<name>``.
    Returns ``"custom"`` if SPARK_HOME is set to an unrecognized path.
    """
    from core.spark_constants import get_spark_home
    spark_home = get_spark_home()
    resolved = spark_home.resolve()

    default_resolved = _get_default_spark_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.spark itself — its parent is ~/ and its
        # directory name is ".spark", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: spark profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.spark itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.spark). "
            "Specify a different name: spark profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for spark profile names."""
    return '''# Spark Agent profile completion
# Add to ~/.bashrc: eval "$(spark completion bash)"

_spark_profiles() {
    local profiles_dir="$HOME/.spark/profiles"
...
complete -F _spark_completion spark
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for spark profile names."""
    return '''#compdef spark
# Spark Agent profile completion
# Add to ~/.zshrc: eval "$(spark completion zsh)"

_spark() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.spark/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.spark/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SPARK_HOME path string.

    Called early in the CLI entry point, before any spark modules
    are imported, to set the SPARK_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: spark profile create {profile_name}"
        )

...
1. **User plugins**   – ``~/.spark/plugins/<name>/``
2. **Project plugins** – ``./.spark/plugins/<name>/`` (opt-in via
   ``SPARK_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``spark_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "spark_agent.plugins"

_NS_PARENT = "spark_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``spark honcho ...``).
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 2. Project plugins (./.spark/plugins/)
        if _env_enabled("SPARK_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".spark" / "plugins"
        ...
        for manifest in manifests:
            ...
            # Validate requires_env before attempting import so missing credentials
            # produce a clear warning rather than a cryptic ImportError or AttributeError.
            missing_env = [
                var for var in (manifest.requires_env or [])
                if not __import__("os").environ.get(var)
            ]
            if missing_env:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = f"missing required env vars: {', '.join(missing_env)}"
                self._plugins[manifest.name] = loaded
                logger.warning(
                    "Skipping plugin '%s' — missing required env vars: %s",
                    manifest.name, ", ".join(missing_env),
                )
                continue
        ...
        if manifests:
            ...
            quarantined = [
                ...
                if p.error and not p.enabled and p.error != "disabled via config"
            ]
            logger.info(
            ...
            if quarantined:
                logger.warning("Quarantined plugins (failed to load): %s", ", ".join(quarantined))
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``spark_plugins.<name>``."""

#!/usr/bin/env python3
"""
SQLite State Store for Spark Agent.
...
class SessionDB:
    ...
    # With multiple spark processes (gateway + CLI sessions + worktree agents)
    ...
    def _init_schema(self):
        ...
        row = cursor.fetchone()
        if row is None:
            # Start at v1 so incremental migrations (v2–v8) run on fresh DBs.
            # Inserting SCHEMA_VERSION directly would skip ALTERs for columns not
            # in the baseline CREATE TABLE (e.g. kanban_status).
        ...
        if row is not None:
            ...
            if current_version < 8:
                ...
                try:
                    cursor.execute(
                        "UPDATE sessions SET kanban_status = 'done' "

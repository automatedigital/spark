"""
Timezone-aware clock for Spark.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``SPARK_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.spark/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Spark never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Spark requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("SPARK_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
    ...
    try:
        ...
        logger.debug("Ignoring error in _resolve_timezone_name()", exc_info=True)

"""ACP session manager — maps ACP sessions to Spark AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.spark/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Spark agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Spark AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Spark runtime provider configuration.
    ...
    def _get_db(self):
        ...
        Note: we resolve ``SPARK_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_spark_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=spark_home / "state.db")
            return self._db_instance
        except Exception:

"""ACP agent server — exposes Spark Agent via the Agent Client Protocol."""
...
try:
    from spark_cli import __version__ as SPARK_VERSION
except Exception:
    SPARK_VERSION = "0.0.0"
...
class SparkACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Spark AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Spark version",
        },
    )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["spark-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Spark using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="spark-agent", version=SPARK_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Spark on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from core.model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["spark-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Spark Agent v{SPARK_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Spark has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:

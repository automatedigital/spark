"""spark-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $SPARK_HOME/config.yaml (profile-scoped):
  plugins:
    spark-memory-store:
...
def _load_plugin_config() -> dict:
    ...
    try:
        ...
        return all_config.get("plugins", {}).get("spark-memory-store", {}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, spark_home):
        """Write config to config.yaml under plugins.spark-memory-store."""
        from pathlib import Path
        config_path = Path(spark_home) / "config.yaml"
        try:
            import yaml
            existing = {}
            ...
            existing["plugins"]["spark-memory-store"] = values
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from core.spark_constants import get_spark_home
        _spark_home = str(get_spark_home())
        _default_db = _spark_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $SPARK_HOME in user-supplied paths so config values like
        # "$SPARK_HOME/memory_store.db" or "~/.spark/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$SPARK_HOME", _spark_home)
            db_path = db_path.replace("${SPARK_HOME}", _spark_home)

...
def _load_supermemory_config(spark_home: str) -> dict:
    config = _default_config()
    config_path = Path(spark_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, spark_home: str) -> None:
    config_path = Path(spark_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `spark memory setup`.
        # All other options are documented for $SPARK_HOME/supermemory.json
    ...
    def save_config(self, values, spark_home):
        ...
        _save_supermemory_config(sanitized, spark_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from core.spark_constants import get_spark_home
        self._spark_home = kwargs.get("spark_home") or str(get_spark_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._spark_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "spark", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "spark_memory", "target": target, "type": "explicit_memory"},

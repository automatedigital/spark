...
Config file: `$SPARK_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `spark` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `spark-{identity}` → `spark-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Spark profile:

```json
{
  "container_tag": "spark-{identity}"
}
```

For a profile named `coder`, this resolves to `spark-coder`. The default profile resolves to `spark-default`. Without `{identity}`, all profiles share the same container.

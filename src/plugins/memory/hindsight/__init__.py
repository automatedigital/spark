...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   — memory bank identifier (default: spark)
  HINDSIGHT_BUDGET    — recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   — API endpoint
  HINDSIGHT_MODE      — cloud or local (default: cloud)

Or via $SPARK_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SPARK_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_spark_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "spark": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "spark"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Spark Agent and the User"
    ...
    def save_config(self, values, spark_home):
        """Write config to $SPARK_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(spark_home) / "hindsight"
    ...
    def post_setup(self, spark_home: str, config: dict) -> None:
        ...
        self.save_config(provider_config, spark_home)

        if env_writes:
            env_path = Path(spark_home) / ".env"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "spark"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Spark Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "spark"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "spark"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("spark", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "spark")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Spark Agent and the User")

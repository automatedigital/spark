...
Handles: spark honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "spark" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Spark profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'spark honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'spark honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `spark update` -- no output, no exceptions.
    """
    try:
        from spark_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Spark profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $SPARK_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_spark_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Spark persistent cross-session memory.")
    ...
    spark_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'spark honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = spark_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        spark_host["peerName"] = new_peer

    current_ai = spark_host.get("aiPeer") or cfg.get("aiPeer", "spark")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        spark_host["aiPeer"] = new_ai

    current_workspace = spark_host.get("workspace") or cfg.get("workspace", "spark")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        spark_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = spark_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        spark_host["observationMode"] = new_obs
    else:
        spark_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(spark_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        spark_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        spark_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = spark_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        spark_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = spark_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        spark_host["sessionStrategy"] = new_strat

    spark_host["enabled"] = True
    spark_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        spark_config = load_config()
        spark_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(spark_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: spark config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    spark honcho status     -- show full config")
    print("    spark honcho mode       -- change recall/observation mode")
    print("    spark honcho tokens     -- tune context and dialectic budgets")
    print("    spark honcho peer       -- update peer names")
    print("    spark honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Spark profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: spark honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'spark honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: spark honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        spark = hosts.get(_host_key(), {})
        user = spark.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = spark.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = spark.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = spark.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Spark' identity in Honcho. Seed with 'spark honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: spark honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = spark.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = spark.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = spark.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Spark asks Honcho's AI peer a question")
        ...
        print("\n  Set with: spark honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'spark honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'spark honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    spark honcho identity --show        — show both peer representations")
        print("    spark honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Spark + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Spark\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Spark persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  spark honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'spark honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'spark honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  spark honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'spark'")
        ...
        if has_key:
            ...
            print("  Run 'spark honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Spark, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'spark honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    spark honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  spark honcho identity <path/to/SOUL.md>")
    ...
    print("    Spark:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Spark:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Spark:   Honcho observes every message and updates representations")
    ...
    print("    Spark:   per-session by default — each run gets its own session")
    print("              Map a custom name:  spark honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. spark honcho setup              — configure API key (required)")
        print("  2. spark honcho migrate            — re-run this walkthrough")
    else:
        print("  1. spark honcho status             — verify Honcho connection")
        print("  2. spark                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. spark honcho identity --show    — verify AI peer representation")
        print("  4. spark honcho tokens             — tune context and dialectic budgets")
        print("  5. spark honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'spark memory setup'...\n")
        from spark_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``spark honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``spark honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to spark memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Spark Honcho",
    )

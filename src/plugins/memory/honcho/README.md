...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/spark>
...
```bash
spark honcho setup    # full interactive wizard (cloud or local)
spark memory setup    # generic picker, also works
```

Or manually:
```bash
spark config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SPARK_HOME/honcho.json` | Profile-local (isolated Spark instances) |
| 2 | `~/.spark/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Spark profile: `spark` (default) or `spark.<profile>`.
...
Host key derivation: `SPARK_HONCHO_HOST` env > active profile (`spark.<profile>`) > `"spark"`.
...
| `SPARK_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `spark honcho setup` | Full interactive setup wizard |
| `spark honcho status` | Show resolved config for active profile |
| `spark honcho enable` / `disable` | Toggle Honcho for active profile |
| `spark honcho mode <mode>` | Change recall or observation mode |
| `spark honcho peer --user <name>` | Update user peer name |
| `spark honcho peer --ai <name>` | Update AI peer name |
| `spark honcho tokens --context <N>` | Set context token budget |
| `spark honcho tokens --dialectic <N>` | Set dialectic max chars |
| `spark honcho map <name>` | Map current directory to a session name |
| `spark honcho sync` | Create host blocks for all Spark profiles |

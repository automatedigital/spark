...
# (spark-* prefixed), and scenario toolsets.
...
def _build_child_progress_callback(task_index: int, parent_agent, task_count: int = 1) -> Callable[..., Any] | None:
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on another provider).
    """
    ...
    # Extract parent's API key so subagents inherit auth when appropriate.
...
def delegate_task(
    ...
    import core.model_tools as _model_tools
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'spark auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (spark_cli/config.py load_config()) so that

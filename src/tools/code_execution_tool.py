...
Lets the LLM write a Python script that calls Spark tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `spark_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `spark_tools.py` with file-based RPC stubs
...
def retry(fn, max_attempts=3, delay=2):
    """Retry a function up to max_attempts times with jittered exponential backoff.
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt < max_attempts - 1:
                ...
                time.sleep(base + _random.uniform(0, 0.5 * base))
    raise last_err
...
"""Auto-generated Spark tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["SPARK_RPC_SOCKET"])
...
"""Auto-generated Spark tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("SPARK_RPC_DIR") or os.path.join(tempfile.gettempdir(), "spark_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated spark_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/spark_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_spark_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/spark_tools.py", tools_src)
        ...
        env_prefix = (
            f"SPARK_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SPARK_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with spark_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="spark_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"spark_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated spark_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_spark_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "spark_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        # --- Spawn child process ---
        # Build an allowlisted environment for the child. Provider API keys,
        # token variables, and credential-file paths loaded into Spark's process
        # env are intentionally absent unless explicitly allowed via
        # env_passthrough. The child accesses tools via RPC, not direct API.
        from tools.env_passthrough import build_tool_subprocess_env
        child_env = build_tool_subprocess_env()
        child_env["SPARK_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the spark-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.
        _spark_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        child_env["PYTHONPATH"] = _spark_root
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.
        _tz_name = os.getenv("SPARK_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name

        # Per-profile HOME isolation: redirect system tool configs into
        # {SPARK_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.spark/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``spark tools`` (e.g. web is turned off),
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from spark_tools import {import_str}` "

...
# All skills live in ~/.spark/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> dict[str, str]:
    """Load profile-scoped environment variables from SPARK_HOME/.env."""
...
def _is_gateway_surface() -> bool:
    if os.getenv("SPARK_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("SPARK_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.spark/.env manually."
...
def _get_category_from_path(skill_path: Path) -> str | None:
    ...
    For paths like: ~/.spark/skills/mlops/axolotl/SKILL.md -> "mlops"
    ...
    try:
        ...
        logger.debug("Ignoring error in _get_category_from_path()", exc_info=True)
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("SPARK_PLATFORM")
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.spark/skills/",
                },
                ensure_ascii=False,
            )
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.debug("Ignoring error in skill_view()", exc_info=True)
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.spark/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `spark skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.spark.* first (agentskills.io convention), fall back to top-level
        spark_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            spark_meta = metadata.get("spark", {}) or {}

        tags = _parse_tags(spark_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            spark_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )

"""
Cron job management tools for Spark Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("SPARK_SESSION_PLATFORM")
    origin_chat_id = get_session_env("SPARK_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("SPARK_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("SPARK_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via spark model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within SPARK_HOME/scripts/.
    ...
    # Only relative paths within ~/.spark/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.spark/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.spark/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_spark_home() / "scripts"
    ...
    try:
        ...
        if normalized == "create":
            ...
            if prompt is not None and not isinstance(prompt, str):
                return tool_error("prompt must be a string", success=False)
            if prompt and len(prompt) > 50_000:
                return tool_error(f"prompt exceeds maximum length (50,000 chars); got {len(prompt)}", success=False)
            if prompt:
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": "Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under ~/.spark/scripts/. On update, pass empty string to clear."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("SPARK_INTERACTIVE")
        or os.getenv("SPARK_GATEWAY_SESSION")
        or os.getenv("SPARK_EXEC_ASK")
    )

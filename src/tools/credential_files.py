...
def register_credential_file(
    ...
    *relative_path* is relative to ``SPARK_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside SPARK_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    spark_home = _resolve_spark_home()

    # Reject absolute paths — they bypass the SPARK_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to SPARK_HOME)",
            relative_path,
        )
        return False

    host_path = spark_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape SPARK_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, spark_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.spark/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="spark-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching spark_constants.get_spark_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_spark_dir()`` for backward compatibility with old directory layouts.
    """

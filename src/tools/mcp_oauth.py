...
This module provides the glue:
    - ``SparkTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Spark Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses SPARK_HOME so each profile gets its own OAuth tokens.
    Layout: ``SPARK_HOME/mcp-tokens/``
    """
    try:
...
# ---------------------------------------------------------------------------
# SparkTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class SparkTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SPARK_HOME/mcp-tokens/<server_name>.json         -- tokens
        SPARK_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Spark.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = SparkTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Spark Agent")
    scope = cfg.get("scope")

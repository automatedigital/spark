...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Spark exit
...
# display_spark_home imported lazily at call site (stale-module safety during spark update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of spark directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "spark-*")):
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (SPARK_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (SPARK_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and os.getenv("SPARK_INTERACTIVE"):
        ...
        if background:
            ...
            try:
                ...
                if notify_on_complete and background:
                    ...
                    _gw_platform = _gse("SPARK_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("SPARK_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("SPARK_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("SPARK_SESSION_USER_ID", "")
                        _gw_user_name = _gse("SPARK_SESSION_USER_NAME", "")

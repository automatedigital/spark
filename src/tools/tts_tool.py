...
Configuration is loaded from ~/.spark/config.yaml under the 'tts:' key.
...
def _get_default_output_dir() -> str:
    ...
    return str(get_spark_dir("cache/audio", "audio_cache"))
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("spark_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.spark/config.yaml (tts: section).
    ...
    platform = get_session_env("SPARK_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'spark-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run spark setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.spark/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }

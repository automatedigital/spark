...
it is automatically downloaded from GitHub releases to $SPARK_HOME/bin/tirith.
...
def _get_spark_home() -> str:
    """Return the Spark home directory, respecting SPARK_HOME env var."""
    return str(get_spark_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_spark_home(), ".tirith-install-failed")
...
def _spark_bin_dir() -> str:
    """Return $SPARK_HOME/bin, creating it if needed."""
    d = os.path.join(_get_spark_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SPARK_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_spark_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SPARK_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SPARK_HOME/bin/tirith
    ...
    spark_bin = os.path.join(_spark_bin_dir(), "tirith")
    if os.path.isfile(spark_bin) and os.access(spark_bin, os.X_OK):
        _resolved_path = spark_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return spark_bin

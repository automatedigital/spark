...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SPARK_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $SPARK_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SPARK_ENV_PATH = (
    r'(?:~\/\.spark/|'
    r'(?:\$home|\$\{home\})/\.spark/|'
    r'(?:\$spark_home|\$\{spark_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart spark-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart spark-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(spark|gateway|cli\.py)\b', "kill.spark/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill spark` but not
    # `kill -9 $(pgrep -f spark)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["SPARK_SPINNER_PAUSE"] = "1"
    try:
        while True:
            print()
        ...
        if "SPARK_SPINNER_PAUSE" in os.environ:
            del os.environ["SPARK_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("SPARK_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("SPARK_INTERACTIVE")
    is_gateway = os.getenv("SPARK_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("SPARK_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("SPARK_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("SPARK_INTERACTIVE")
    is_gateway = os.getenv("SPARK_GATEWAY_SESSION")
    is_ask = os.getenv("SPARK_EXEC_ASK")

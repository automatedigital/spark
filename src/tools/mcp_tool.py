...
discovers their tools, and registers them into the spark-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.spark/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            spark_home = os.path.expanduser(
                os.getenv(
                    "SPARK_HOME", os.path.join(os.path.expanduser("~"), ".spark")
                )
            )
            candidates = [
                os.path.join(spark_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            # 2. Remove old tools from spark-* umbrella toolsets
            for ts_name, ts in TOOLSETS.items():
                if ts_name.startswith("spark-"):
...
def _load_mcp_config() -> dict[str, dict]:
    """Read ``mcp_servers`` from the Spark config file.
    ...
    ``os.environ`` (which includes ``~/.spark/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Spark's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Spark registry schema format.
...
def _sync_mcp_toolsets(server_names: list[str] | None = None) -> None:
    """Expose each MCP server as a standalone toolset and inject into spark-* sets.
    ...
    Also injects all MCP tools into spark-* umbrella toolsets for the
    default behavior.
    ...
    # Also inject into spark-* umbrella toolsets for default behavior.
    for ts_name, ts in TOOLSETS.items():
        if not ts_name.startswith("spark-"):
            continue
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> list[str]:
    ...
    and spark-* umbrella toolset injection.
    ...
    if registered_names:
        ...
        # Inject into spark-* umbrella toolsets for default behavior
        for ts_name, ts in TOOLSETS.items():
            if ts_name.startswith("spark-"):
...
def probe_mcp_server_tools() -> dict[str, list[tuple]]:
    ...
    Designed for ``spark tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Spark registry.

#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Spark Skills Hub.
...
Used by spark_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    DEFAULT_TAPS = [
        ...
        {"repo": "heygen-com/hyperframes", "path": "skills/"},
    ]
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Automate Digital) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.spark/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def __init__(self):
        ...
        self._optional_dir = get_optional_skills_dir(
            Path(__file__).parent.parent / "skills" / "optional"
        )
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                spark_meta = meta_block.get("spark", {})
                if isinstance(spark_meta, dict):
                    tags = spark_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Spark centralized index source
# ---------------------------------------------------------------------------

SPARK_INDEX_URL = "https://spark.automatedigital.ai/docs/api/skills-index.json"
SPARK_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "spark-index.json"
SPARK_INDEX_TTL = 6 * 3600  # 6 hours


def _load_spark_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SPARK_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if SPARK_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - SPARK_INDEX_CACHE_FILE.stat().st_mtime
    ...
    try:
        resp = httpx.get(SPARK_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Spark index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Spark index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class SparkIndexSource(SkillSource):
    """Skill source backed by the centralized Spark Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "spark-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "spark-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SparkIndexSource(auth=auth), # Centralized index (search + resolved install paths)

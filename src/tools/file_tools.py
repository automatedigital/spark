...
def _check_sensitive_path(filepath: str) -> str | None:
    ...
    # Allow writes inside the OS temp directory (e.g. macOS /var/folders/...)
    # so normal temp-file workflows and tests are not blocked.
    try:
        temp_root = os.path.realpath(tempfile.gettempdir())
    except (OSError, ValueError):
        temp_root = ""
    if temp_root:
        if resolved == temp_root or resolved.startswith(temp_root + os.sep):
            return None
...
def read_file_tool(
    ...
    try:
        ...
        # ── Spark internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from core.spark_constants import get_spark_home as _get_hh

        _spark_home = _get_hh().resolve()
        _blocked_dirs = [
            _spark_home / "skills" / ".hub" / "index-cache",
            _spark_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps(
                    {
                        "error": (
                            f"Access denied: {path} is an internal Spark cache file "

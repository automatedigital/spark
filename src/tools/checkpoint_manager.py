...
Architecture:
    ~/.spark/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        SPARK_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_spark_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("SPARK_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> str | None:
    ...
    _run_git(["config", "user.email", "spark@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Spark Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "SPARK_WORKDIR").write_text(

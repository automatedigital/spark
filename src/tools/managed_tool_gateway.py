"""Generic managed-tool gateway helpers for Spark Portal-hosted vendor passthroughs."""
...
def auth_json_path():
    """Return the Spark auth store path, respecting SPARK_HOME overrides."""
...
def read_nous_access_token() -> Optional[str]:
    """Read a Spark Portal Subscriber OAuth access token from auth store or env override."""
    ...
    try:
        from spark_cli.auth import resolve_nous_access_token
        ...
        logger.debug("Spark Portal access token refresh failed: %s", exc)
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and Spark Portal access token are available."""

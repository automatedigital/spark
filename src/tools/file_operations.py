...
def _get_safe_write_root() -> str | None:
    """Return the resolved SPARK_WRITE_SAFE_ROOT path, or None if unset.
    ...
    """
    root = os.getenv("SPARK_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
class NativeFileOperations(ShellFileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content", file_glob: str | None = None, limit: int = 50, offset: int = 0, output_mode: str = "content", context: int = 0) -> SearchResult:
        try:

...
# Path to tinker-atropos submodule (relative to spark-agent root)
SPARK_ROOT = Path(__file__).parent.parent
TINKER_ATROPOS_ROOT = SPARK_ROOT / "tinker-atropos"
ENVIRONMENTS_DIR = TINKER_ATROPOS_ROOT / "tinker_atropos" / "environments"
CONFIGS_DIR = TINKER_ATROPOS_ROOT / "configs"
LOGS_DIR = get_spark_home() / "logs" / "rl_training"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.spark/.env",
        }, indent=2)
...
async def rl_check_status(run_id: str) -> str:
    ...
    try:
        ...
        api = wandb.Api()
        runs = api.runs(
            f"{os.getenv('WANDB_ENTITY', 'automatedigital')}/{run_state.wandb_project}",

...
def _load_spark_env_vars() -> dict[str, str]:
    """Load ~/.spark/.env values without failing Docker command execution."""
    try:
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.spark/sandboxes/). Non-persistent
        ...
        container_name = f"spark-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _SPARK_PROVIDER_ENV_BLOCKLIST)
        spark_env = _load_spark_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = spark_env.get(key)

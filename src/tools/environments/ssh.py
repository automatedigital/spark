...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "spark-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.spark"),
    ...
    def _establish_connection(self):
        ...
        try:
            ...
            raise RuntimeError(f"SSH connection to {self.user}@{self.host} timed out") from err
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.spark directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.spark"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="spark-ssh-bulk-") as staging:
            ...
            try:
                ...
                tar_proc.wait()
                ssh_proc.wait()
                raise RuntimeError("SSH bulk upload timed out") from err

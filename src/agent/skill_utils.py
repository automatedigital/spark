...
def get_disabled_skill_names(platform: str | None = None) -> set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SPARK_PLATFORM`` or
            ``SPARK_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> list[Path]:
    ...
    paths that resolve to the local ``~/.spark/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def extract_skill_conditions(frontmatter: dict[str, Any]) -> dict[str, list]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    spark = metadata.get("spark") or {}
    if not isinstance(spark, dict):
        spark = {}
    return {
        "fallback_for_toolsets": spark.get("fallback_for_toolsets", []),
        "requires_toolsets": spark.get("requires_toolsets", []),
        "fallback_for_tools": spark.get("fallback_for_tools", []),
        "requires_tools": spark.get("requires_tools", []),
    }

...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from spark_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from spark_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from spark_cli.auth import is_provider_explicitly_configured
...
def _try_openrouter() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if pool_present:
        ...
        return OpenAI(
            api_key=or_key, base_url=base_url,
...
def _try_custom_endpoint() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    return (
        OpenAI(api_key=custom_key, base_url=custom_base, **httpx_client_kwargs()),
        model,
    )
...
def _try_codex() -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = OpenAI(api_key=codex_token, base_url=base_url, **httpx_client_kwargs())
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `spark model` but the
    #    old OPENAI_BASE_URL lingers in ~/.spark/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: spark model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.spark/.env",
...
def _to_async_client(sync_client, model: str):
    ...
    async_kwargs.update(httpx_client_kwargs(async_mode=True))
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: spark model)")
                return None, None
    ...
    try:
        from spark_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("spark_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from spark_cli.models import _should_use_copilot_responses_api

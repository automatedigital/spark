...
APP_CREATION_GUIDANCE = (
    ...
    "subdirectory name for the project. If Spark is running in a TUI or local "
    "session already opened inside a specific project directory and that directory "
    "is clearly the intended target, work there instead.\n"
    "For user-created web projects, use available design skills such as `design-md` "
    "and `frontend-design` when they match the request.\n"
    "Do not edit Spark's built-in web dashboard or Web UI for normal user-created "
    "app/dashboard requests. The built-in dashboard lives under `src/spark_cli/web/` "
    "and `src/spark_cli/web_server.py`; only modify those dashboard/frontend server "
    "files when the user explicitly asks to change Spark's own built-in dashboard "
    "or Web UI.\n"
...
def _skills_prompt_snapshot_path() -> Path:
    return get_spark_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    _platform_hint = (
        os.environ.get("SPARK_PLATFORM")
        or get_session_env("SPARK_SESSION_PLATFORM")
        or ""
    )
    cache_key = (
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills\n"
            "Before replying, check the list below. If a skill matches your task, "
            "load it with skill_view(name) and follow its instructions — skills encode "
            "specialized workflows, API details, and the user's preferred approach. "
            "Err on the side of loading. Fix outdated skills with skill_manage(action='patch').\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Spark subscription capability block for the system prompt."""
    try:
        from spark_cli.nous_subscription import get_nous_subscription_features
        from tools.tool_backend_helpers import managed_nous_tools_enabled
    except Exception as exc:
        logger.debug("Failed to import Spark subscription helper: %s", exc)
        return ""
    ...
    def _status_line(feature) -> str:
        if feature.managed_by_nous:
            return f"- {feature.label}: active via Spark subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.nous_auth_present:
            return f"- {feature.label}: included with Spark subscription, not currently selected"
        if feature.key == "modal" and features.nous_auth_present:
            return f"- {feature.label}: optional via Spark subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Spark Subscription",
        "Spark subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Spark-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that Spark subscription would unlock or simplify, suggest Spark subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: spark setup, spark setup tools, spark setup terminal, spark status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    ...
    try:
        ...
        logger.debug("Could not ensure SPARK_HOME before loading SOUL.md: %s", e)
...
def _load_spark_md(cwd_path: Path) -> str:
    """.spark.md / SPARK.md — walk to git root."""
    spark_md_path = _find_spark_md(cwd_path)
    if not spark_md_path:
        return ""
    try:
        content = spark_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = spark_md_path.name
        try:
        ...
        logger.debug("Could not read %s: %s", spark_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .spark.md / SPARK.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SPARK_HOME is independent and always included when present.
    ...
    # SOUL.md from SPARK_HOME only — skip when already loaded as identity
    if not skip_soul:

...
# Minimum context length required to run Spark Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Spark from probing-down to
        ...
        with httpx.Client(timeout=3.0) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Spark grow conversations past the runtime limit and Ollama
...
def _resolve_nous_context_length(model: str) -> Optional[int]:
    """Resolve Spark Portal model context length via OpenRouter metadata.

    Spark Portal model IDs are bare (e.g. 'claude-opus-4-6') while OpenRouter uses
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Spark sends to providers:

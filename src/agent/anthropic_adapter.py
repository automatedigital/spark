"""Anthropic Messages API adapter for Spark Agent.

Translates between Spark's internal OpenAI-style message format and
...
def resolve_anthropic_token() -> str | None:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Spark)
    ...
    # 1. Spark-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Spark configs.
...
# ── Spark-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.spark/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_SPARK_OAUTH_FILE = get_spark_home() / ".anthropic_oauth.json"
...
def run_spark_oauth_login_pure() -> dict[str, Any] | None:
    ...
    print()
    print("Authorize Spark with your Claude Pro/Max subscription.")
    print()
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Spark Agent", "Claude Code")
                text = text.replace("Spark agent", "Claude Code")
                text = text.replace("spark-agent", "claude-code")
                text = text.replace("Automate Digital", "Anthropic")
                block["text"] = text

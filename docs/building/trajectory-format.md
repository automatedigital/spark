...
Spark Agent saves conversation trajectories in ShareGPT-compatible JSONL format — ready to use as training data, debugging artifacts, or reinforcement learning datasets.

Source files: `agent/trajectory.py`, `run_agent.py` (search for `_save_trajectory`), `batch_runner.py`


## Where trajectories are written
...
The batch runner (`batch_runner.py`) writes to a custom output file per batch (e.g., `batch_001_output.jsonl`) with additional metadata fields. You can override the filename via the `filename` parameter in `save_trajectory()`.


## JSONL entry format

Each line is a self-contained JSON object. Two variants exist:
...
`tool_stats` and `tool_error_counts` are normalized to include ALL possible tools (from `model_tools.TOOL_TO_TOOLSET_MAP`) with zero defaults, so every entry has the same schema. This prevents Arrow schema mismatch errors when loading into HuggingFace datasets.
...
- Arguments parse from JSON strings back to objects (not double-encoded)
- If JSON parsing fails, an empty `{}` is used with a warning logged
...
- The tool name matches by position against the parent assistant's `tool_calls` array

### System message

The system message is generated at save time (not taken from the conversation). It follows the Spark function-calling prompt template with:
...
Trajectories are standard JSONL — load them with any JSON-lines reader:
...
### Loading into HuggingFace Datasets
...
Or pass `--save-trajectories` on the command line. When `save_trajectories=True`, `_save_trajectory()` runs at the end of each conversation turn.

The batch runner always saves trajectories — that's its primary purpose. Samples with zero reasoning across all turns are automatically discarded by the batch runner to keep training data clean.

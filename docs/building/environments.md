...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Spark-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Spark Agent's environment framework connects its tool-calling capabilities to the [Atropos](https://github.com/AutomateDigital/atropos) RL training framework. This unlocks three workflows from a single codebase:

1. **RL Training** — Train language models on multi-turn agentic tasks with GRPO
2. **Benchmarks** — Evaluate models on standardized agentic benchmarks
...
The Python environment framework documented here lives under `environments/` and is the implementation-level API for Spark/Atropos integration. This is separate from the user-facing `rl_*` tools, which are an orchestration surface for remote RL training workflows.
:::
...
The environment system is a three-layer inheritance chain:
...
classDiagram
    ...
    BaseEnv <|-- SparkAgentBaseEnv
    SparkAgentBaseEnv <|-- TerminalTestEnv
    SparkAgentBaseEnv <|-- SparkSweEnv
    SparkAgentBaseEnv <|-- TerminalBench2EvalEnv
...
- **Wandb integration** — metrics logging and rollout visualization
- **CLI interface** — three subcommands: `serve`, `process`, `evaluate`
- **Eval logging** — `evaluate_log()` saves results to JSON + JSONL

### SparkAgentBaseEnv

The Spark-specific layer (`environments/spark_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls Spark's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `SparkAgentLoop` and scores the result
- **Two-phase operation** — Phase 1 (OpenAI server) for eval/SFT, Phase 2 (VLLM ManagedServer) for full RL with logprobs
- **Async safety patches** — monkey-patches the Modal backend to work inside Atropos's event loop

### Concrete environments

Your environment inherits from `SparkAgentBaseEnv` and implements five methods:
...
`SparkAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as Spark's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any Spark tool |
...
parser = get_parser("spark")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `spark`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.

In Phase 1, parsers are not needed — the server handles tool call parsing natively.
...
Dataset: [AutomateDigital/terminal-bench-2](https://huggingface.co/datasets/AutomateDigital/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [AutomateDigital/openthoughts-tblite](https://huggingface.co/datasets/AutomateDigital/openthoughts-tblite).
...
pip install "spark-agent[yc-bench]"
...
YC-Bench uses [collinear-ai/yc-bench](https://github.com/collinear-ai/yc-bench) — a deterministic simulation with 4 skill domains (research, inference, data_environment, training), a prestige system, employee management, and financial pressure. Unlike TB2's per-task binary scoring, YC-Bench measures whether an agent can maintain coherent strategy across hundreds of compounding decisions.

## Training environments

### TerminalTestEnv

A minimal self-contained environment with inline tasks (no external dataset). Use it to **validate the full stack** end-to-end. Each task asks the model to create a file at a known path; the verifier checks the content.
...
```bash
python environments/spark_swe_env/spark_swe_env.py serve \
...
Runs all items, computes metrics, logs to WandB. No training server needed.
...
Each output line is a scored trajectory: full conversation history, reward, and metadata.

### `serve` — Connect to Atropos for RL training

Connects the environment to a running Atropos API server. Use this during live RL training.
...
- Placeholder tokens are created for the Atropos pipeline (real token IDs aren't available from the OpenAI API)
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"spark"`, `"qwen"`, `"mistral"`)
...
```python
from environments.spark_base_env import SparkAgentBaseEnv, SparkAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(SparkAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(SparkAgentBaseEnv):
    name = "my-env"
...
4. **Implement** `rollout_and_score_eval(eval_item)` — per-item agent loop + scoring
...
8. **Run with** the `evaluate` subcommand

See `environments/benchmarks/yc_bench/yc_bench_env.py` for a clean, well-documented reference implementation.

## Configuration reference

### SparkAgentEnvConfig fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which Spark toolsets to enable |
...
| `tool_call_parser` | `str` | `"spark"` | Parser for Phase 2 raw output |
| `extra_body` | `Dict` | `None` | Extra params for OpenAI API (e.g., OpenRouter provider prefs) |
| `eval_handling` | `Enum` | `STOP_TRAIN` | `STOP_TRAIN`, `LIMIT_TRAIN`, `NONE` |

### YAML configuration

Pass a config file with `--config` to avoid long CLI arguments:
...
env:
  ...
  dataset_name: "AutomateDigital/terminal-bench-2"
  tokenizer_name: "AutomateDigital/Spark-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/AutomateDigital/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "spark-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "spark-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
  spark_base_env.py          # Abstract base class (SparkAgentBaseEnv)
  agent_loop.py              # Multi-turn agent engine (SparkAgentLoop)
  tool_context.py            # Per-rollout tool access for reward functions
  patches.py                 # Async-safety patches for Modal backend

  tool_call_parsers/         # Phase 2 client-side parsers
    spark_parser.py          # Spark/ChatML <tool_call> format
  ...
  spark_swe_env/             # SWE-bench training environment

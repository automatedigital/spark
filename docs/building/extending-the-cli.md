...
description: "Build wrapper CLIs that extend the Spark TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

You can add panels, hotkeys, and layout customizations to Spark's TUI without touching the `run()` method. `SparkCLI` exposes five protected extension hooks your wrapper CLI can override. Your extension stays decoupled from Spark's internals.

## Extension points at a glance

| Hook | What you gain | Override when... |
|------|-------------|------------------|
...
```python
#!/usr/bin/env python3
"""my_cli.py - Example wrapper CLI that extends Spark."""

from cli import SparkCLI
from prompt_toolkit.layout import FormattedTextControl, Window
from prompt_toolkit.filters import Condition


class MyCLI(SparkCLI):
...
Returns a list of prompt_toolkit widgets to insert into the TUI layout. Widgets appear **between the spacer and the status bar** — above the input area, below the main output.
...
Each widget should be a prompt_toolkit container (`Window`, `ConditionalContainer`, `HSplit`). Use `ConditionalContainer` or `filter=Condition(...)` to make widgets toggleable:
...
Called after Spark registers its own keybindings and before the layout is built. Add your bindings to `kb`.
...
**Avoid conflicts** with built-in keybindings: `Enter` (submit), `Escape Enter` (newline), `Ctrl-C` (interrupt), `Ctrl-D` (exit), `Tab` (auto-suggest accept). Function keys F2+ and most Ctrl-combinations are safe.

### `_build_tui_layout_children(**widgets)`

Override this only when you need full control over widget ordering. For most extensions, `_get_extra_tui_widgets()` is the right choice.
...
Top to bottom, the default layout is:
...
- **Force a redraw** after state changes: call `self._invalidate()` to trigger a prompt_toolkit refresh.
- **Access agent state**: `self.agent`, `self.model`, and `self.conversation_history` are all available on `self`.

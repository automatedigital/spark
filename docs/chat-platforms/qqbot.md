...
Connect Spark to QQ via the **Official QQ Bot API (v2)**. Supports private (C2C) messages, group @-mentions, guild messages, direct messages, and voice transcription.
...
1. **QQ Bot Application** — Register at [q.qq.com](https://q.qq.com):
   ...
   - Configure your bot in sandbox mode for testing, then publish for production
...
Add your credentials to `~/.spark/.env`:
...
For fine-grained control, add platform settings to `~/.spark/config.yaml`:
...
Voice messages are transcribed in two stages:

1. **QQ built-in ASR** (free, always tried first) — QQ provides `asr_refer_text` in voice message attachments via Tencent's own speech recognition
2. **Configured STT provider** (fallback) — If QQ's ASR returns nothing, the adapter calls an OpenAI-compatible STT API:
...
- **Sandbox-only bot** — In sandbox mode, the bot only receives messages from QQ's sandbox test channel
...
- Confirm `aiohttp` and `httpx` are installed: `pip install aiohttp httpx`
- Check network connectivity to `api.sgroup.qq.com` and the WebSocket gateway
- Review gateway logs for error messages and reconnect behavior

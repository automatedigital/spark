...
Connect Spark to WeCom (Enterprise WeChat) as a self-built enterprise application. Unlike the bot mode, your app shows up as a first-class entry in users' WeCom sidebars and supports routing across multiple corps.

:::info WeCom Bot vs WeCom Callback
Spark supports two WeCom integration modes:
- **[WeCom Bot](wecom.md)** — bot-style, connects via WebSocket. Simpler setup, works in group chats.
- **WeCom Callback** (this page) — self-built app, receives encrypted XML callbacks. First-class sidebar presence. Supports multi-corp routing.
:::
...
3. Spark decrypts the message and queues it for the agent
4. Spark immediately acknowledges (silently — nothing shown to the user yet)
5. The agent processes the request (typically 3–30 seconds)
...
The callback adapter starts an HTTP server on the configured port. WeCom verifies your callback URL via a GET request, then starts sending messages via POST.
...
Running multiple self-built apps — across departments or subsidiaries — is straightforward. Configure the `apps` list in `config.yaml`:
...
Users are scoped by `corp_id:user_id` to prevent cross-corp collisions. When a message arrives, the adapter records which app (corp) the user belongs to and routes replies through the correct app's access token.
...
All callback payloads use AES-CBC encryption with the EncodingAESKey. The adapter handles both directions:
...
- **Text only** — currently supports text input; image/file/voice input is not yet implemented. The agent is aware of outbound media capabilities via the WeCom platform hint (images, documents, video, voice)
- **Response latency** — agent sessions typically take 3–30 seconds; users see the reply when processing completes

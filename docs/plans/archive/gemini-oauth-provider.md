...
## Client ID
- Need to register a "Desktop app" OAuth client on a Automate Digital GCP project
...
## Token Lifecycle
- Store at `~/.spark/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `spark_cli/auth.py` - Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `spark_cli/models.py` - Add Gemini model catalog
4. `spark_cli/runtime_provider.py` - Add gemini branch (read OAuth token, build OpenAI client)
5. `spark_cli/main.py` - Add `_model_flow_gemini()`, add to provider choices
6. `spark_cli/setup.py` - Add gemini auth flow (trigger browser OAuth)
...
### Docs
12. `docs/getting-started/quickstart.md` - Add gemini to provider table
13. `docs/configuration.md` - Gemini setup section
14. `docs/reference/environment-variables.md` - New env vars

## Estimated scope
~400 lines new code, ~150 lines modifications, ~100 lines tests, ~50 lines docs = ~700 lines total

## Prerequisites
- Automate Digital GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via SPARK_GEMINI_CLIENT_ID env var
...
- spark-agent Copilot OAuth: `spark_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)

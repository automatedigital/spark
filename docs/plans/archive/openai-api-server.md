# OpenAI-Compatible API Server for Spark Agent
...
SSE streaming. By exposing this endpoint, spark-agent becomes instantly
...
```
  ...
  AnythingLLM           {"messages": [...]}               spark-agent   
  NextChat                                                gateway        
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.spark/.env`
2. Run `spark gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with spark-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns spark-agent as a model) |
...
Authorization: Bearer spark-api-key-here
...
But spark-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "spark-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured spark-agent model.
...
1. `gateway/platforms/api_server.py` - new adapter
   ...
   - Frontend system prompt layered on top of spark-agent's core prompt
...
Once implemented, spark-agent works as a drop-in backend for:

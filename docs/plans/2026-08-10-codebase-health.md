...
**Done.** The `pytest` job runs
`python -m pytest tests/ -m "not slow and not integration" -q --timeout=60`.
Adding it first required fixing 8 pre-existing failures on
...
**Done: 54 of 123 routes**, in thirteen new modules — `cron`, `logs`, `analytics`,
...
- [x] Add per-provider golden transcript coverage before extracting the
  orchestration loop required to remain in the facade by ADR 0012
- [x] Reduce `src/core/run_agent/__init__.py` — **11,341 -> 1,744** while
...
`AIAgent` gained six mixins — `tool_execution`, `codex_streaming`,
`agent_session`, `agent_context`, `agent_memory`, and `agent_support` — plus
`stream_events` and `qwen_headers` for two helpers both
...
`run_conversation` and its immediate orchestration helpers now live in the
`turn_loop` mixin. Byte-exact provider request/transcript goldens cover standard
chat completions, Anthropic Messages, and Codex Responses before that boundary
is crossed, satisfying ADR 0012. The constructor remains in the facade: moving
it changed the resolved tool-schema profile, which the cache golden caught.
...
- [x] Gate `UP015` and apply the safe bulk modernization pass
- [x] Clear the 14 remaining `UP028`/`UP031`/`UP035`/`UP037`/`UP042` findings

**Final state.** The ratchet is
`ruff check src/ --select UP015,F,B,I,W,UP028,UP031,UP035,UP037,UP042 --ignore B027`.
All selected findings are cleared. The remaining Ruff findings are outside the
ratchet and include deliberate late-import patterns used during bootstrap.
...
- [x] Replace the remaining bare `except Exception: pass` handlers

**Done: 955 of 955.** Every tracked source handler whose body was exactly
`pass` now records a debug traceback. The final four were early gateway
bootstrap handlers; moving logger construction above bootstrap made them safe
to instrument and also fixed a latent `NameError` when `certifi` is absent.
...
- [x] Clear the 15 modules still listed in the mypy overrides block

**Done.** `mypy src/agent/ src/spark_cli/` checks all 126 modules and exits
clean. The 15-module `ignore_errors` override was removed, so the CI command
now gates the full declared scope.
...
The final pass also exposed two runtime bugs: the update prefetch helper was
incorrectly decorated as an async context manager even though it is called as
a normal warm-up function, and token estimation omitted the provider required
by `get_model_capabilities`, silently falling back to a 200K context window.
Both are fixed and covered by the clean type boundary.
...
- [x] Reduce `api.ts` below 800 lines — now **487**
- [ ] Reduce `ChatPanel.tsx` below 800 lines

**api.ts: 3,052 -> 487.** It was one file holding a 1,456-line `api` object of
...
also verifies recovery after a backend restart and runs
`e2e/chat-contracts.mjs` for transcript, action, reconnect, switching, stop,
and redirect contracts. Both pass locally in about 45 seconds total. They
need `PYTHON=python` because the
...
| 4 | Finish `run_agent` + `cli` splits | **11,341 -> 1,744 and 4,101 -> 1,134** | constructor decomposition only |
| 5 | Widen CI ratchet | **Done** | — |
| 6 | Log swallowed exceptions | **Done** (955 of 955) | — |
| 7 | `mypy` to zero | **Done** (126 modules clean and gated) | — |
| 8 | Unblock the event loop | **Withdrawn — false finding** | — |
| 9 | Split large React files | **api.ts done** (3,052 -> 487) | 2 days for ChatPanel |
...
### Fixed: test isolation in the web-turn module

`test_conversation_message_continues_latest_compressed_leaf` previously skipped
on CI when another test's process-wide runtime state caused its work future to
cancel before execution. The web-event fixture now owns a fresh `AsyncRuntime`
per test and shuts it down after draining active turns. The CI-only skip has
been removed; the async-runtime, scheduler, and full web-event sequence passes
with `CI=true` (124 tests).

## Work completed

Branch `fix/plan-quality-gates`. The committed head checks were green before
the cleanup pass. Current local acceptance evidence is recorded in the final
handoff: 12,288 safe Python tests and 369 frontend tests pass, Ruff and mypy
are clean, the production bundle is within budget, both browser E2E suites
pass, and the 251-entry route table has no duplicate method/path pairs.
...
- Python: 12,287 safe fast/non-integration tests

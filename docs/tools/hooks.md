...
Spark gives you two ways to run custom code at key lifecycle points:
...
Gateway hooks fire automatically during gateway operation (Telegram, Discord, Slack, WhatsApp) without touching the main agent pipeline.

### Create a Hook

Each hook is a directory under `~/.spark/hooks/` containing two files:
...
The `events` list determines which events trigger your handler. Subscribe to any combination, including wildcards like `command:*`.
...
LOG_FILE = Path.home() / ".spark" / "hooks" / "my-hook" / "activity.log"
...
Register for `command:*` and your handler fires for every `command:` event (`command:model`, `command:reset`, etc.). Monitor all slash commands with a single subscription.

### Examples

#### Boot Checklist (BOOT.md) — Built-in

The gateway ships with a built-in `boot-md` hook that looks for `~/.spark/BOOT.md` at every startup. If the file exists, the agent runs its instructions in a background session. No installation needed — just create the file.

**Create `~/.spark/BOOT.md`:**

```markdown
# Startup Checklist

1. Check if any cron jobs failed overnight - run `spark cron list`
...
The agent runs these in a background thread so gateway startup isn't blocked. If nothing needs attention, the agent replies with `[SILENT]` and no message is delivered.
...
```yaml
# ~/.spark/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.spark/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.spark/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.spark/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".spark" / "logs" / "command_usage.jsonl"
...
POST to an external service when a new session starts:

```yaml
# ~/.spark/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.spark/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/spark-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.spark/hooks/`
...
- If a callback **crashes**, it's logged and skipped. Other hooks and the agent continue normally.
...
**Use cases:** Logging, audit trails, tool call counters, rate limiting, printing warnings before dangerous operations.
...
**Fires:** In `model_tools.py`, inside `handle_function_call()`, after the tool's handler returns. Fires once per tool call. Does **not** fire if the tool raised an unhandled exception — the error is caught and returned as an error JSON string, and `post_tool_call` fires with that error string as `result`.

**Return value:** Ignored.

**Use cases:** Logging tool results, metrics collection, tracking tool success/failure rates, notifications when specific tools finish.
...
**Return value:** Return a dict with a `"context"` key, or a plain non-empty string, to append text to the current turn's user message. Return `None` for no injection.

```python
# Inject context
return {"context": "Recalled memories:\n- User likes Python\n- Working on spark-agent"}
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns so cached tokens are reused. Plugins contribute context alongside the user's input, not by modifying Spark's core system prompt.
...
Fires **once per turn**, after the tool-calling loop completes and the agent produces a final response. Only fires on **successful** turns — does not fire if the turn was interrupted.
...
**Fires:** In `run_agent.py`, inside `run_conversation()`, after the tool loop exits with a final response. Guarded by `if final_response and not interrupted` — does not fire when the user interrupts mid-turn or the agent hits the iteration limit without a response.
...
Fires **once** when a brand-new session is created. Does **not** fire when the user sends a second message in an existing session.
...
2. **`cli.py`** — in the CLI's atexit handler, but **only** if the agent was mid-turn (`_agent_running=True`) when the exit occurred. Catches Ctrl+C and `/exit` during processing. In this case, `completed=False` and `interrupted=True`.

...
- Give it a name like "spark-agent"
- Select scopes:
...
- Give it a title like "spark-agent-<machine-name>"
...
elif [ -f ~/.spark/.env ] && grep -q "^GITHUB_TOKEN=" ~/.spark/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.spark/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"

...
version: 1.0.0
author: Siddharth Balyan + Spark Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Spark. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.spark/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.spark/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:AutomateDigital" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.spark/.env`, make sure `~/.config/x-cli/.env` still points at the current file.

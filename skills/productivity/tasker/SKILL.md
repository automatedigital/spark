---
name: tasker
description: Ticket-driven dev workflow backed by your tracker (Notion, Jira, Linear, GitHub Issues, or any MCP-connected tracker). Use when the user types /tasker, /tasker connect, /tasker plan, /tasker status, /tasker resume, /tasker sync, /tasker brainstorm, or /tasker decide. Connects to a ticket backend, brainstorms and triages ideas, plans tickets, then builds feature branches and opens PRs — keeping the tracker and GitHub in sync.
version: 1.0.0
author: spark
license: MIT
metadata:
  spark:
    tags: [Tasks, Planning, GitHub, Notion, Productivity]
---

# Tasker

Bundled `/tasker` ticket-driven workflow for the current repository. The
...
Projects may override this bundled skill with a project-local `tasker` skill
when they need repository-specific planner rules or ticket schemas.

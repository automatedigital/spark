...
- `gws_bridge.py` refreshes the Spark token and injects it into `gws` via `GOOGLE_WORKSPACE_CLI_TOKEN`
...
GWORKSPACE_SKILL_DIR="$SPARK_HOME/skills/productivity/google-workspace"
PYTHON_BIN="${SPARK_PYTHON:-python3}"
if [ -x "$SPARK_HOME/spark-agent/venv/bin/python" ]; then
  PYTHON_BIN="$SPARK_HOME/spark-agent/venv/bin/python"
fi

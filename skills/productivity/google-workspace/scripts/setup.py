#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Spark Agent.
...
try:
    ...
    SPARK_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if SPARK_AGENT_ROOT.exists():
        sys.path.insert(0, str(SPARK_AGENT_ROOT))
    from core.spark_constants import display_spark_home, get_spark_home

SPARK_HOME = get_spark_home()
TOKEN_PATH = SPARK_HOME / "google_token.json"
CLIENT_SECRET_PATH = SPARK_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = SPARK_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Spark profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Spark home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_spark_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Spark")

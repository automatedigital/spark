#!/usr/bin/env python3
"""Google Workspace API CLI for Spark Agent.

Thin wrapper that delegates to gws (googleworkspace/cli) via gws_bridge.py.
Maintains the same CLI interface for backward compatibility with Spark skills.
...
def gws(*args: str) -> None:
    ...
    result = subprocess.run(
        ...
        env={**os.environ, "SPARK_HOME": os.environ.get("SPARK_HOME", str(Path.home() / ".spark"))},
    )
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Spark Agent (gws backend)")

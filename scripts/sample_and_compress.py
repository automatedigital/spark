...
DEFAULT_DATASETS = [
    "AutomateDigital/swe-terminus-agent-glm-kimi-minimax",
    "automatedigital/spark-megascience-sft1",
    "automatedigital/spark-Thinking-GLM-4.7-SFT2",
    "automatedigital/spark-Thinking-GLM-4.7-SFT1",
    "AutomateDigital/terminal-tasks-glm-spark-agent"
]
...
def load_dataset_from_hf(dataset_name: str) -> List[Dict[str, Any]]:
    ...
    Args:
        dataset_name: HuggingFace dataset name (e.g., "AutomateDigital/dataset-name")
...
def sample_from_datasets(
    ...
    print(f"\nIN Loading {len(datasets)} datasets...")
    ...
    for dataset_name in datasets:
        ...
        if not entries:
            print(f"   WARN  Skipping {dataset_name} (no entries loaded)")
            continue
    ...
    print(f"\nSTATS Total entries loaded: {len(all_entries):,}")
    
    # Filter by token count using parallel processing
    print(f"\nSEARCH Filtering trajectories with >= {min_tokens:,} tokens (using {num_proc} workers)...")
    ...
    if len(filtered_entries) <= total_samples:
        print(f"\nWARN  Only {len(filtered_entries):,} trajectories available, using all of them")
...
def save_samples_for_compression(
    ...
    print(f"\nSAVE Saving {len(samples)} samples to {output_dir}")
...
def run_compression(input_dir: Path, output_dir: Path, config_path: str):
    ...
    print(f"\nZIP  Running trajectory compression...")
...
def merge_output_to_single_jsonl(input_dir: Path, output_file: Path):
    ...
    """
    print(f"\nOTHER Merging output files into {output_file.name}...")
...
def main(
    ...
    """
    print("=" * 70)
    print("STATS TRAJECTORY SAMPLING AND COMPRESSION")
    print("=" * 70)
    ...
    if not skip_download:
        ...
        if not samples:
            print("ERROR No samples collected. Exiting.")
            return
        
        # Step 2: Save to JSONL files
        save_samples_for_compression(samples, sampled_dir, batch_size)
    else:
        print(f"\nSKIP  Skipping download, using existing data in {sampled_dir}")
    ...
    print("OK COMPLETE!")
    print("=" * 70)
    print(f"\nPATH Raw samples:        {sampled_dir}")
    print(f"PATH Compressed batches: {compressed_dir}")
    print(f"PATH Final output:       {final_output}")
    print(f"\nTo upload to HuggingFace:")
    print(f"   huggingface-cli upload AutomateDigital/{output_name} {final_output}")

#!/usr/bin/env python3
"""Build the Spark Skills Index - a centralized JSON catalog of all skills.
...
`spark skills search/install` can use it without hitting the GitHub API.
...
# Ensure SPARK_HOME is set (needed by tools/skills_hub.py imports)
os.environ.setdefault("SPARK_HOME", os.path.join(os.path.expanduser("~"), ".spark"))
...
def batch_resolve_paths(skills: list, auth: GitHubAuth) -> list:
    ...
    Instead of resolving each skill individually (NM API calls), we:
...
def main():
    print("Building Spark Skills Index...", flush=True)

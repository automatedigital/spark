 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.spark/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.spark', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.spark', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.spark', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.spark', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '*Spark Agent*\n------------\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Spark Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (qr) {
      console.log('\nPHONE Scan this QR code with WhatsApp on your phone:\n');
    ...
    if (connection === 'close') {
      ...
      if (reason === DisconnectReason.loggedOut) {
        ...
        if (reason === 515) {
          console.log('retry WhatsApp requested restart (code 515). Reconnecting...');
        } else {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Spark' own reply messages in self-chat mode to avoid loops.
...
if (PAIR_ONLY) {
  ...
  console.log('PHONE WhatsApp pairing mode');
  console.log(`PATH Session: ${SESSION_DIR}`);
  console.log();
  startSocket();
} else {
  app.listen(PORT, '127.0.0.1', () => {
    console.log(`BRIDGE WhatsApp bridge listening on port ${PORT} (mode: ${WHATSAPP_MODE})`);
    console.log(`PATH Session stored in: ${SESSION_DIR}`);
    if (ALLOWED_USERS.size > 0) {
      console.log(`LOCK Allowed users: ${Array.from(ALLOWED_USERS).join(', ')}`);
    } else {

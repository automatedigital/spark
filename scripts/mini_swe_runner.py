#!/usr/bin/env python3
"""
SWE Runner with Spark Trajectory Format

A runner that uses Spark-Agent's built-in execution environments
(local, docker, modal) and outputs trajectories in the Spark-Agent format
compatible with batch_runner.py and trajectory_compressor.py.

Features:
- Uses Spark-Agent's Docker, Modal, or Local environments for command execution
- Outputs trajectories in Spark format (from/value pairs with <tool_call>/<tool_response> XML)
...
# ============================================================================
# Terminal Tool Definition (matches Spark-Agent format)
# ============================================================================
...
def create_environment(
    ...
    **kwargs
    ...
    """
    Create an execution environment using Spark-Agent's built-in backends.
...
# ============================================================================
# Mini-SWE Runner with Spark Trajectory Format
# ============================================================================

class MiniSWERunner:
    """
    Agent runner that uses Spark-Agent's built-in execution environments
    and outputs trajectories in Spark-Agent format.
    """
    ...
    def __init__(
        ...
        print("AGENT Mini-SWE Runner initialized")
    ...
    def _create_env(self):
        ...
        print(f"TOOL Creating {self.env_type} environment...")
    ...
    def _convert_to_spark_format(
        ...
        """
        Convert internal message format to Spark trajectory format.
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        """
        print(f"\n{'='*60}")
        print(f"NOTE Task: {task[:80]}{'...' if len(task) > 80 else ''}")
        print(f"{'='*60}")
...
Complete the user's task step by step."""
        ...
        try:
            while api_call_count < self.max_iterations:
                api_call_count += 1
                print(f"\nBG API call #{api_call_count}/{self.max_iterations}")
                ...
                if assistant_message.content:
                    print(f"AGENT Assistant: {assistant_message.content[:100]}...")
                
                # Check for tool calls
                if assistant_message.tool_calls:
                    print(f"TOOL Tool calls: {len(assistant_message.tool_calls)}")
                    ...
                    for tc in assistant_message.tool_calls:
                        ...
                        print(f"   CALL terminal: {command[:60]}...")
            ...
            if api_call_count >= self.max_iterations:
                print(f"WARN  Reached max iterations ({self.max_iterations})")
        
        finally:
            # Cleanup environment
            self._cleanup_env()
        
        # Convert to Spark trajectory format
        trajectory = self._convert_to_spark_format(messages, task, completed)
    ...
    def run_batch(
        ...
        print(f"\nOTHER Running batch of {len(prompts)} tasks")
        print(f"PATH Output: {output_file}")
        
        with open(output_file, 'w', encoding='utf-8') as f:
            for i, prompt in enumerate(prompts, 1):
                print(f"\n{'='*60}")
                print(f"TASK Task {i}/{len(prompts)}")
                print(f"{'='*60}")
...
def main(
    ...
    """
    Run SWE tasks with Spark trajectory format output.
    ...
    """
    print("RUN Mini-SWE Runner with Spark Trajectory Format")
    print("=" * 60)
    ...
    if task:
        ...
        print(f"\nPATH Trajectory saved to: {output_file}")
        print(f"OK Completed: {result['completed']}")
        print(f"CALL API calls: {result['api_calls']}")
        print(f"MSG Turns: {len(result['conversations'])}")
        ...
        print("ERROR Please provide either --task or --prompts_file")

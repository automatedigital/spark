"""
Spark MCP Server - expose messaging conversations as MCP tools.
...
Plus: channels_list (Spark-specific extra)

Usage:
    spark mcp serve
    spark mcp serve --verbose
...
logger = logging.getLogger("spark.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SPARK_HOME."""
    try:
        from core.spark_constants import get_spark_home
        return get_spark_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SPARK_HOME", Path.home() / ".spark")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from core.spark_constants import get_spark_home
        directory_file = get_spark_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SPARK_HOME", Path.home() / ".spark")
...
class EventBridge:
    ...
    This is the Spark equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        """
        # Check if sessions.json has changed (mtime check is ~1s)
        sessions_file = _get_sessions_dir() / "sessions.json"
        try:
        ...
        try:
            from core.spark_constants import get_spark_home
            db_file = get_spark_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SPARK_HOME", Path.home() / ".spark")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Spark MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'spark-agent[mcp]'"
        )

    mcp = FastMCP(
        "spark",
        instructions=(
            "Spark Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Spark MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

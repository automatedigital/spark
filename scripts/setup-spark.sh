...
# 5. Symlinks the 'spark' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}Spark Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        echo -e "${GREEN}OK${NC} uv found ($UV_VERSION)"
    else
        ...
        if curl -LsSf https://astral.sh/uv/install.sh | sh 2>/dev/null; then
            ...
            if [ -n "$UV_CMD" ]; then
                UV_VERSION=$($UV_CMD --version 2>/dev/null)
                echo -e "${GREEN}OK${NC} uv installed ($UV_VERSION)"
            else
                echo -e "${RED}ERROR${NC} uv installed but not found. Add ~/.local/bin to PATH and retry."
                exit 1
            fi
        else
            echo -e "${RED}ERROR${NC} Failed to install uv. Visit https://docs.astral.sh/uv/"
            exit 1
        fi
    fi
fi
...
if is_termux; then
    if command -v python >/dev/null 2>&1; then
        ...
        if "$PYTHON_PATH" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 11) else 1)' 2>/dev/null; then
            ...
            echo -e "${GREEN}OK${NC} $PYTHON_FOUND_VERSION found"
        else
            echo -e "${RED}ERROR${NC} Termux Python must be 3.11+"
            echo "    Run: pkg install python"
            exit 1
        fi
    else
        echo -e "${RED}ERROR${NC} Python not found in Termux"
        echo "    Run: pkg install python"
        exit 1
    fi
else
    if $UV_CMD python find "$PYTHON_VERSION" &> /dev/null; then
        ...
        PYTHON_PATH=$($UV_CMD python find "$PYTHON_VERSION")
        PYTHON_FOUND_VERSION=$($PYTHON_PATH --version 2>/dev/null)
        echo -e "${GREEN}OK${NC} $PYTHON_FOUND_VERSION installed"
    fi
fi
...
if is_termux; then
    ...
    if [ -f "constraints-termux.txt" ]; then
        "$SETUP_PYTHON" -m pip install -e ".[termux]" -c constraints-termux.txt || {
            echo -e "${YELLOW}WARN${NC} Termux bundle install failed, falling back to base install..."
...
if is_termux; then
    ...
    $UV_CMD pip install -e "./tinker-atropos" && \
        echo -e "${GREEN}OK${NC} tinker-atropos installed" || \
        echo -e "${YELLOW}WARN${NC} tinker-atropos install failed (RL tools may not work)"
else
    echo -e "${YELLOW}WARN${NC} tinker-atropos not found (run: git submodule update --init --recursive)"
fi
...
if command -v rg &> /dev/null; then
    echo -e "${GREEN}OK${NC} ripgrep found"
else
    echo -e "${YELLOW}WARN${NC} ripgrep not found (file search will use grep fallback)"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if [ "$INSTALLED" = true ]; then
            echo -e "${GREEN}OK${NC} ripgrep installed"
        else
...
# ============================================================================
# PATH setup - symlink spark into a user-facing bin dir
# ============================================================================
...
echo -e "${GREEN}OK${NC} Symlinked spark -> $COMMAND_LINK_DISPLAY_DIR/spark"

if is_termux; then
    export PATH="$COMMAND_LINK_DIR:$PATH"
    echo -e "${GREEN}OK${NC} $COMMAND_LINK_DISPLAY_DIR is already on PATH in Termux"
else
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                ...
                echo -e "${GREEN}OK${NC} Added ~/.local/bin to PATH in $SHELL_CONFIG"
            else
                echo -e "${GREEN}OK${NC} ~/.local/bin already in $SHELL_CONFIG"
            fi
        else
            echo -e "${GREEN}OK${NC} ~/.local/bin already on PATH"
        fi
    fi
fi

# ============================================================================
# Seed bundled skills into ~/.spark/skills/
# ============================================================================

SPARK_SKILLS_DIR="${SPARK_HOME:-$HOME/.spark}/skills"
mkdir -p "$SPARK_SKILLS_DIR"
...
echo "Other commands:"
echo "  spark status        # Check configuration"
if is_termux; then
    echo "  spark gateway       # Run gateway in foreground"
else
    echo "  spark gateway install # Install gateway service (messaging + cron)"
fi
echo "  spark cron list     # View scheduled jobs"
echo "  spark doctor        # Diagnose issues"
echo ""

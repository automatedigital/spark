...
VERSION_FILE = REPO_ROOT / "spark_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "teknium@automatedigital.ai": "teknium1",
    ...
    "shannon@automatedigital.ai": "shannonsands",
    ...
    "spark@automatedigital.ai": "AutomateDigital",
    ...
    "ben@automatedigital.ai": "benbarclay",
    ...
    "emozilla@automatedigital.ai": "emozilla",
    ...
    "jonny@automatedigital.ai": "jquesnelle",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "spark@automatedigital.ai"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/automatedigital/spark",
    ...
    lines.append(f"# Spark Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Spark Agent. See below for everything included in this initial release.")
        lines.append("")
    ...
    category_order = [
        ("breaking", "WARN Breaking Changes"),
        ("features", " Features"),
        ("improvements", "TOOL Improvements"),
        ("fixes", "BUG Bug Fixes"),
        ("docs", "DOCS Documentation"),
        ("tests", "TEST Tests"),
        ("chore", "INFRA Infrastructure"),
...
def main():
    parser = argparse.ArgumentParser(description="Spark Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Spark Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        if args.bump:
            ...
            if add_result.returncode != 0:
                print(f"  ERROR Failed to stage version files: {add_result.stderr.strip()}")
                return
            ...
            if commit_result.returncode != 0:
                print(f"  ERROR Failed to commit version bump: {commit_result.stderr.strip()}")
                return
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Spark Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        if push_result.returncode == 0:
            ...
            print(f"  ERROR Failed to push to origin: {push_result.stderr.strip()}")
        ...
        gh_cmd = [
            ...
            "--title", f"Spark Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            if result is None:
                print("  ERROR GitHub release skipped: `gh` CLI not found.")
            else:
                print(f"  ERROR GitHub release failed: {result.stderr.strip()}")
            print(f"    Release notes kept at: {changelog_file}")
            print(f"    Tag was created locally. Create the release manually:")
            print(
                f"    gh release create {tag_name} --title 'Spark Agent v{new_version} ({calver_date})' "

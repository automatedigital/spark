# ============================================================================
# Spark Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/automatedigital/spark/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$SparkHome = "$env:LOCALAPPDATA\spark",
    [string]$InstallDir = "$env:LOCALAPPDATA\spark\spark-agent"
)
...
$RepoUrlSsh = "git@github.com:automatedigital/spark.git"
$RepoUrlHttps = "https://github.com/automatedigital/spark.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             Spark Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Automate Digital.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    $managedNode = "$SparkHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$SparkHome\node;$env:Path"
        Write-Success "Node.js $version found (Spark-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.spark/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\spark-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$SparkHome\node") { Remove-Item -Recurse -Force "$SparkHome\node" }
                Move-Item $extractedDir.FullName "$SparkHome\node"
                $env:Path = "$SparkHome\node;$env:Path"

                $version = & "$SparkHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.spark/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/automatedigital/spark/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\spark-agent-$Branch.zip"
                $extractPath = "$env:TEMP\spark-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up spark command..."
    
    if ($NoVenv) {
        $sparkBin = "$InstallDir"
    } else {
        $sparkBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so spark is globally available
    # On Windows, the spark.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$sparkBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$sparkBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $sparkBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SPARK_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\spark instead
    # of the Unix default ~/.spark
    $currentSparkHome = [Environment]::GetEnvironmentVariable("SPARK_HOME", "User")
    if (-not $currentSparkHome -or $currentSparkHome -ne $SparkHome) {
        [Environment]::SetEnvironmentVariable("SPARK_HOME", $SparkHome, "User")
        Write-Success "Set SPARK_HOME=$SparkHome"
    }
    $env:SPARK_HOME = $SparkHome
    
    # Update current session
    $env:Path = "$sparkBin;$env:Path"
    
    Write-Success "spark command ready"
}
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$SparkHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SparkHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$SparkHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.spark/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.spark/.env"
        }
    } else {
        Write-Info "~/.spark/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$SparkHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.spark/config.yaml from template"
        }
    } else {
        Write-Info "~/.spark/config.yaml already exists, keeping it"
    }
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
    ...
    Write-Success "Configuration directory ready: ~/.spark/"
    
    # Seed bundled skills into ~/.spark/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.spark/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.spark/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SparkHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.spark/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m spark_cli.main setup
    } else {
...
function Start-GatewayIfConfigured {
    ...
    $sparkCmd = "$InstallDir\venv\Scripts\spark.exe"
    if (-not (Test-Path $sparkCmd)) {
        $sparkCmd = "spark"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SparkHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'spark whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SparkHome\logs\gateway.log"
            Start-Process -FilePath $sparkCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SparkHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: spark gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: spark gateway"
    }
}
...
function Write-Completion {
    ...
    # Show file locations
    Write-Host "PATH Your files:" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "   Config:    " -NoNewline -ForegroundColor Yellow
    Write-Host "$SparkHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SparkHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SparkHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SparkHome\spark-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   spark              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   spark setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   spark config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   spark config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   spark gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   spark update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/automatedigital/spark/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
